...
function extractReferenceData($) {
  // Extract all subdivision options
  const subdivisions = [];
  $('#subdivision option[data-tokens]').each((i, el) => {
    const value = $(el).attr('value');
    const name = $(el).attr('data-tokens');
    const text = textClean($(el).text());
    if (value && name) {
      subdivisions.push({ code: value, name: name, display_text: text });
    }
  });

  // Extract all city options
  const cities = [];
  $('.search_name_city option[data-tokens]').each((i, el) => {
    const value = $(el).attr('value');
    const name = $(el).attr('data-tokens');
    if (value && name && value !== '*') {
      cities.push({ value: value, name: name });
    }
  });

  // Extract address city options
  $('.search_address_city option[data-tokens]').each((i, el) => {
    const value = $(el).attr('value');
    const name = $(el).attr('data-tokens');
    if (value && name && value !== '*') {
      const exists = cities.find(c => c.value === value && c.name === name);
      if (!exists) {
        cities.push({ value: value, name: name });
      }
    }
  ...
  // Extract taxing district options
  const taxing_districts = [];
  $('select[name="taxingdistrict"] option[data-tokens]').each((i, el) => {
    const value = $(el).attr('value');
    const name = $(el).attr('data-tokens');
    const text = textClean($(el).text());
    if (value && name) {
      taxing_districts.push({ code: value, name: name, display_text: text });
    }
  });

  // Extract year options
  const years = [];
  $('.form-control.form-control-sm.picker option').each((i, el) => {
    const value = $(el).attr('value');
    const text = textClean($(el).text());
...
function extractTableData($) {
  ...
  // Extract all table data from div.table-wrapper tables
  $('.table-wrapper table.table tbody tr').each((i, tr) => {
    const rowData = [];
    $(tr).find('td').each((j, td) => {
      const text = textClean($(td).text());
      if (text) {
        rowData.push(text);
      }
    });
...
function extractTabContent($) {
  // Extract tab content including Address tab
  const tabs = {};

  // Address tab content
  const addressTab = $('#tab-Address');
  if (addressTab.length) {
    tabs.address = {
      id: addressTab.attr('id'),
      text: textClean(addressTab.text()),
      href: addressTab.attr('href')
    };
  }
  ...
  if (ownerTab.length) {
    tabs.owner = {
      id: ownerTab.attr('id'),
      text: textClean(ownerTab.text()),
      href: ownerTab.attr('href')
    };
  }

  // Parcel tab content
  const parcelTab = $('#tab-Parcel');
  if (parcelTab.length) {
    tabs.parcel = {
      id: parcelTab.attr('id'),
      text: textClean(parcelTab.text()),
      href: parcelTab.attr('href')
    };
  }

  // Subdivision tab content
  const subdivisionTab = $('#tab-Subdivision');
  if (subdivisionTab.length) {
    tabs.subdivision = {
      id: subdivisionTab.attr('id'),
      text: textClean(subdivisionTab.text()),
      href: subdivisionTab.attr('href')
    };
  }
...
function extractMiscellaneousData($) {
  const misc = {};

  // Extract census block
  $('.details-card table.putnam-table tbody tr').each((i, tr) => {
    const heading = textClean($(tr).find('td.row-heading').text());
    const value = textClean($(tr).find('td').last().text());
    if (heading && value) {
      if (/census\s+block/i.test(heading)) {
        misc.census_block = value;
      } else if (/location/i.test(heading)) {
        misc.location = value;
      } else if (/neighborhood/i.test(heading)) {
        misc.neighborhood = value;
      }
    }
  });
...
function main() {
  ...
  // Extract and write reference data
  const refData = extractReferenceData($);
  writeOut("reference_subdivisions.json", refData.subdivisions);
  writeOut("reference_cities.json", refData.cities);
  writeOut("reference_taxing_districts.json", refData.taxing_districts);
  writeOut("reference_years.json", refData.years);

  // Extract and write table data
  const tableData = extractTableData($);
  writeOut("reference_table_data.json", tableData);

  // Extract and write tab content
  const tabContent = extractTabContent($);
  writeOut("reference_tabs.json", tabContent);

  // Extract and write miscellaneous data
  const miscData = extractMiscellaneousData($);
  writeOut("reference_miscellaneous.json", miscData);

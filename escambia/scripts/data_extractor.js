 ...
 */
function createParcelGeometries(geometries, dataDir) {
  ...
  geometries.forEach((geom, geomIndex) => {
    ...
    fs.writeFileSync(
      path.join(dataDir, geometryFile),
    ...
    fs.writeFileSync(
      path.join(dataDir, relationshipFile),
 ...
 * @param {string} dataDir - Data directory path
 */
function createLayoutGeometries(csvContent, dataDir) {
  const rows = parseCsv(csvContent.replace(NORMALIZE_EOL_REGEX, '\n'));
...
function main() {
  ...
  const geometry = {
    ...
    latitude: unnormalizedFromInput?.latitude ?? null,
    longitude: unnormalizedFromInput?.longitude ?? null
  };
  fs.writeFileSync(
    path.join(dataDir, "geometry.json"),
    JSON.stringify(geometry, null, 2),
  );
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(relAddressGeometry, null, 2),
  );

  // 2. Create parcel.json
  const parcel = {
    source_http_request: seed.source_http_request || {
      method: "GET",
      url: ""
  ...
  // 3. Create parcel polygon geometries from CSV (if available)
  const geometryCsv = loadGeometryCsvContent();
  if (geometryCsv) {
    try {
      const instances = createGeometryInstances(geometryCsv);
      if (instances.length) {
        createParcelGeometries(instances, dataDir);
    ...
    // 4. Create layout/building polygon geometries from CSV (if available)
    try {
      createLayoutGeometries(geometryCsv, dataDir);

...
function formatRawAddressForOutput(address) {
  ...
  const copyField = (field, transform) => {
    ...
    result[field] = stringified;
  };

  copyField("latitude", (value) => parseCoordinate(value));
  copyField("longitude", (value) => parseCoordinate(value));
  copyField("city_name", (value) => sanitizeCityName(value));
  copyField("state_code", (value) =>
    value ? String(value).trim().toUpperCase() : null,
  );
  copyField("postal_code", (value) => sanitizePostalCode(value));
  copyField("plus_four_postal_code", (value) => sanitizePlus4(value));
  copyField("country_code", (value) =>
    value ? String(value).trim().toUpperCase() : null,
  );
  copyField("county_name", (value) => {
    const titled = toTitleCase(String(value));
    return titled && titled.trim().length ? titled : null;
  });
  copyField("municipality_name", (value) => {
    const titled = toTitleCase(String(value));
    return titled && titled.trim().length ? titled : null;
  });

  const optionalGridFields = [
    "route_number",
    "township",
    "range",
    "section",
  ...
  for (const field of optionalGridFields) {
  ...
  if (
    !Object.prototype.hasOwnProperty.call(result, "postal_code") &&
...
async function main() {
  ...
  if (locationLine || normalizedCity || resolvedState || postalCode || unnormalizedAddressCandidate) {
    ...
    let materializedRawAddress = null;
    if (rawAddressIsValid) {
      const rawOutput = {};
      ...
      const trimmedRawUnnormalized =
        rawAddressCandidate.unnormalized_address.trim();
      if (trimmedRawUnnormalized.length) {
    ...
    if (finalAddress && finalAddressVariant === "raw") {
      ...
      const pickCoordinate = (field) => {
        for (const bucket of sources) {
          if (
            ...
            const numeric = parseCoordinate(bucket[field]);
            if (numeric != null) {
              return numeric;
            }
          }
      ...
      const rawCandidate = {};
      const unnormalizedPrimary =
        typeof trimmedUnnormalized === "string" && trimmedUnnormalized.length
          ? trimmedUnnormalized
          : null;
      const unnormalizedFallback = pickValue("unnormalized_address");
      const resolvedUnnormalized =
        unnormalizedPrimary ||
        (typeof unnormalizedFallback === "string" ? unnormalizedFallback : null);
      if (resolvedUnnormalized && resolvedUnnormalized.trim().length) {
        rawCandidate.unnormalized_address = resolvedUnnormalized.trim();
      }
      ...
      for (const field of optionalFields) {
        if (Object.prototype.hasOwnProperty.call(rawCandidate, field)) continue;
        switch (field) {
          case "plus_four_postal_code":
          ...
          case "municipality_name":
            ...
            break;
          case "street_suffix_type":
            setField(field, (value) => mapStreetSuffixType(value) || null);
            break;
          case "street_pre_directional_text":
          case "street_post_directional_text":
            setField(field, (value) => String(value).trim().toUpperCase());
            break;
          default:
            setField(field);
        }
      ...
      if (!rawCandidate.country_code && rawCandidate.state_code) {
        rawCandidate.country_code = "US";
      }
      if (!rawCandidate.postal_code && rawCandidate.plus_four_postal_code) {

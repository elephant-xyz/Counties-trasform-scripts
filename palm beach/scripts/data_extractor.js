...
function finalizeCountyAddressNormalization(addressFilePath, options = {}) {
  ...
  if (hasMeaningfulAddressValue(resolvedRaw)) {
    rawSurface.unnormalized_address = resolvedRaw;
  } else if (!rawSurface.unnormalized_address) {
    return;
  }
...
function ensureRawVariantCriticalStrings(address, options = {}) {
  if (!address || typeof address !== "object") return;
  ...
  for (const field of RAW_VARIANT_SCHEMA_FIELDS) {
    ensureFieldPresence(field);
  }
  ensureFieldPresence("plus_four_postal_code");
  ensureFieldPresence("route_number");
  ensureFieldPresence("unit_identifier");

  const uniqueRawStrings = [];
  const seenRaw = new Set();
  for (const candidate of rawCandidates) {
    if (typeof candidate !== "string") continue;
    const trimmed = candidate.trim();
    if (!trimmed.length || seenRaw.has(trimmed)) continue;
    seenRaw.add(trimmed);
    uniqueRawStrings.push(trimmed);
  }

  const assignIfMissing = (field, value, formatter) => {
    if (RAW_ADDRESS_EXCLUDED_FIELDS.has(field)) return;
    if (hasMeaningfulAddressValue(address[field])) return;
    if (value === undefined || value === null) return;
    const prepared =
      typeof formatter === "function" ? formatter(value) : value;
    const normalized = normalizeAddressFieldForSchema(field, prepared);
    if (normalized === undefined || normalized === null) return;
    address[field] = normalized;
  };

  const assignFromArray = (field, values, formatter) => {
    if (!Array.isArray(values) || !values.length) return;
    for (const candidate of values) {
      if (candidate === undefined || candidate === null) continue;
      assignIfMissing(field, candidate, formatter);
      if (hasMeaningfulAddressValue(address[field])) break;
    }
  };

  for (const raw of uniqueRawStrings) {
    const streetSegment = raw.includes(",")
      ? raw.split(",")[0].trim()
      : raw;
    const parsedStreet = parseLocationAddress(streetSegment);
    ...
    const trailingSegments = raw.includes(",")
      ? raw.split(",").slice(1).join(" ").trim()
      : "";
    const cityStateSource = trailingSegments.length ? trailingSegments : raw;
    const parsedCityState = parseCityStatePostal(cityStateSource);

    assignIfMissing(
      "city_name",
      parsedCityState.city,
      (value) => sanitizeCityName(value) || value,
    );
    assignIfMissing(
      "state_code",
    ...
    const postalPieces = extractPostalPieces(raw);
    if (postalPieces) {
  ...
  if (!hasMeaningfulAddressValue(address.street_name)) {
    for (const raw of uniqueRawStrings) {
      ...
      if (!cleaned) continue;
      const formatted = formatStreetNameCase(cleaned);
      if (formatted && formatted.trim().length) {
        address.street_name = formatted.toUpperCase();
        break;
      }
    }
  }
  ...
  if (
    ...
    const tokens = address.street_name.split(/\s+/).filter(Boolean);
    if (tokens.length > 1) {
      const suffixCandidate = tokens[tokens.length - 1];
      const mapped = mapStreetSuffixType(suffixCandidate);
      if (mapped) {
        address.street_suffix_type = mapped;
        address.street_name = tokens.slice(0, -1).join(" ");
      }
    }
  }

  assignFromArray("city_name", cityFallbacks, sanitizeCityName);
  assignFromArray("municipality_name", municipalityFallbacks, toTitleCase);
  assignFromArray("postal_code", postalFallbacks);
  assignFromArray("plus_four_postal_code", plusFourFallbacks);

  if (
    !hasMeaningfulAddressValue(address.municipality_name) &&
    hasMeaningfulAddressValue(address.city_name)
  ) {
    address.municipality_name = toTitleCase(address.city_name);
  }

  if (!hasMeaningfulAddressValue(address.county_name) && defaultCounty) {
    address.county_name = toTitleCase(defaultCounty);
  }
  if (!hasMeaningfulAddressValue(address.state_code) && defaultState) {
    address.state_code = defaultState;
  }
  if (!hasMeaningfulAddressValue(address.country_code) && defaultCountry) {
    address.country_code = defaultCountry;
  }

  const gridLengths = { township: 2, range: 2, section: 2, block: 3, lot: 4 };
  for (const candidate of parcelCandidates) {
    const parsed = deriveGridPartsFromPcn(candidate);
    if (!parsed || typeof parsed !== "object") continue;
    assignIfMissing("township", parsed.township);
    assignIfMissing("range", parsed.range);
    assignIfMissing("section", parsed.section);
    assignIfMissing("block", parsed.block);
    assignIfMissing("lot", parsed.lot);
  }
  for (const [field, length] of Object.entries(gridLengths)) {
  ...
  if (coordinateFallback && typeof coordinateFallback === "object") {
    const latitude = parseCoordinate(coordinateFallback.latitude);
    const longitude = parseCoordinate(coordinateFallback.longitude);
    if (!Number.isFinite(address.latitude) && Number.isFinite(latitude)) {
      address.latitude = latitude;
    }
    if (!Number.isFinite(address.longitude) && Number.isFinite(longitude)) {
      address.longitude = longitude;
    }
  }
...
function solidifyRawAddressFinalOutput(addressFilePath, options = {}) {
  ...
  const coordinateFallback =
    coordinateQueue.find((candidate) => {
  ...
  ensureRawVariantCriticalStrings(rawOutput, {
    rawCandidates: [
      trimmedUnnormalized,
      ...(Array.isArray(rawStrings) ? rawStrings : []),
    ],
    parcelCandidates: parcelQueue,
    defaultCounty: defaultCountyName,
    defaultState: defaultStateCode,
    defaultCountry: defaultCountryCode,
    cityFallbacks: mergedFallbacks.city_name || [],
    municipalityFallbacks: mergedFallbacks.municipality_name || [],
    postalFallbacks: mergedFallbacks.postal_code || [],
    plusFourFallbacks: mergedFallbacks.plus_four_postal_code || [],
    coordinateFallback,
  });
...
function enforceCountyAddressNormalizedVariant(addressFilePath, options = {}) {
  if (!addressFilePath || !fs.existsSync(addressFilePath)) {
    return;
  }
  ...
  for (const field of NORMALIZED_ADDRESS_FIELDS) {
    const value = Object.prototype.hasOwnProperty.call(payload, field)
      ? payload[field]
      : null;
    normalizedSurface[field] = sanitizeAddressFieldValue(field, value);
  }

  const applyFieldCandidate = (field, candidate) => {
    if (candidate === undefined || candidate === null) {
      return false;
    }
    if (hasMeaningfulAddressValue(normalizedSurface[field])) {
  ...
  for (const source of fieldSources) {
    for (const field of NORMALIZED_ADDRESS_FIELDS) {
      if (!Object.prototype.hasOwnProperty.call(source, field)) continue;
      applyFieldCandidate(field, source[field]);
    }
  }
  ...
  for (const [field, fallbackValue] of Object.entries(fieldFallbacks)) {
    ...
    const queue = Array.isArray(fallbackValue)
      ...
      : fallbackValue != null
        ...
        : [];
    for (const candidate of queue) {
      if (applyFieldCandidate(field, candidate)) {
        break;
      }
    }
  }
  ...
  if (Array.isArray(options.coordinateCandidates)) {
    for (const candidate of options.coordinateCandidates) {
      if (!candidate || typeof candidate !== "object") continue;
      if (!hasMeaningfulAddressValue(normalizedSurface.latitude)) {
        applyFieldCandidate("latitude", candidate.latitude);
      }
      if (!hasMeaningfulAddressValue(normalizedSurface.longitude)) {
        applyFieldCandidate("longitude", candidate.longitude);
      }
    }
  }

  if (Array.isArray(options.parcelIdCandidates)) {
    for (const parcelCandidate of options.parcelIdCandidates) {
      if (!parcelCandidate) continue;
      const parsedGrid =
        parseGridFromPcn(parcelCandidate) ||
        deriveGridPartsFromPcn(parcelCandidate);
      ...
      for (const [field, value] of Object.entries(parsedGrid)) {
        applyFieldCandidate(field, value);
      }
    }
  }

  const applyStreetFromParsed = (parsed) => {
    if (!parsed || typeof parsed !== "object") return;
    applyFieldCandidate("street_number", parsed.streetNumber);
    applyFieldCandidate("street_name", parsed.streetName);
    applyFieldCandidate(
      "street_pre_directional_text",
      parsed.streetPreDirectional,
    );
    ...
    applyFieldCandidate("street_suffix_type", parsed.streetSuffix);
    applyFieldCandidate("unit_identifier", parsed.unitIdentifier);
    applyFieldCandidate("route_number", parsed.routeNumber);
  };

  if (Array.isArray(options.streetCandidates)) {
    for (const candidate of options.streetCandidates) {
      if (typeof candidate !== "string") continue;
      const trimmed = candidate.includes(",")
        ? candidate.split(",")[0].trim()
        : candidate.trim();
      if (!trimmed.length) continue;
      applyStreetFromParsed(parseLocationAddress(trimmed));
    }
  }
  ...
  if (
    ...
    normalizedSurface.county_name = toTitleCase(options.defaultCountyName);
  }
  ...
  if (
    ...
    normalizedSurface.state_code = options.defaultStateCode;
  }
  ...
  if (Array.isArray(options.unnormalizedCandidates)) {
    rawQueue.push(...options.unnormalizedCandidates);
  }
  const resolvedRaw = resolveFirstNonEmptyString(rawQueue);
  const trimmedRaw =
    typeof resolvedRaw === "string" ? resolvedRaw.trim() : "";
  if (trimmedRaw.length) {
    normalizedProbe.unnormalized_address = trimmedRaw;
  } else if (
    Object.prototype.hasOwnProperty.call(
      normalizedProbe,
      "unnormalized_address",
  ...
  if (
    requestIdentifierCandidates.some(
  ...
  const preparedSource = resolvedSourceHttpRequest
    ? prepareSourceHttpRequest(resolvedSourceHttpRequest)
    : null;
  if (
    sourceHttpRequestCandidates.some(
      (candidate) => candidate && typeof candidate === "object",
    )
  ) {
    normalizedProbe.source_http_request = preparedSource
      ? deepClone(preparedSource)
      : null;
  } else if (
    Object.prototype.hasOwnProperty.call(
...
function finalizeCountyAddressVariantSimple(addressFilePath, options = {}) {
  if (!addressFilePath || !fs.existsSync(addressFilePath)) {
    return;
  }
  ...
  const hasMeaningful = (field) => hasMeaningfulAddressValue(normalized[field]);

  const setFieldIfMeaningful = (field, value) => {
    if (hasMeaningful(field)) return;
    const sanitized = sanitizeAddressFieldValue(field, value);
    if (hasMeaningfulAddressValue(sanitized)) {
  ...
  const combinedFieldSources = [
    payload,
    ...(Array.isArray(fieldSources) ? fieldSources : []),
    ...(ADDRESS_FALLBACK_CONTEXT &&
    Array.isArray(ADDRESS_FALLBACK_CONTEXT.fieldSources)
  ...
  for (const source of combinedFieldSources) {
    for (const field of NORMALIZED_ADDRESS_FIELDS) {
      if (!Object.prototype.hasOwnProperty.call(source, field)) continue;
      setFieldIfMeaningful(field, source[field]);
    }
  }

  const combinedFallbacks = {};
  const mergeFallbacks = (map) => {
    if (!map || typeof map !== "object") return;
    for (const [field, candidates] of Object.entries(map)) {
      if (!NORMALIZED_ADDRESS_FIELDS.includes(field)) continue;
      if (!combinedFallbacks[field]) {
        combinedFallbacks[field] = [];
      }
      if (Array.isArray(candidates)) {
        combinedFallbacks[field].push(...candidates);
      } else {
        combinedFallbacks[field].push(candidates);
      }
    }
  ...
  mergeFallbacks(fieldFallbacks);
  if (
    ADDRESS_FALLBACK_CONTEXT &&
    ADDRESS_FALLBACK_CONTEXT.fieldFallbacks
  ) {
    mergeFallbacks(ADDRESS_FALLBACK_CONTEXT.fieldFallbacks);
  }

  for (const [field, candidates] of Object.entries(combinedFallbacks)) {
    if (hasMeaningful(field)) continue;
    for (const candidate of candidates) {
      setFieldIfMeaningful(field, candidate);
      if (hasMeaningful(field)) break;
    }
  }
  ...
  for (const candidate of combinedCoordinates) {
    if (!candidate || typeof candidate !== "object") continue;
    if (!hasMeaningful("latitude")) {
      const latitude = parseCoordinate(candidate.latitude);
      if (Number.isFinite(latitude)) {
        normalized.latitude = latitude;
      }
    }
    if (!hasMeaningful("longitude")) {
      const longitude = parseCoordinate(candidate.longitude);
      if (Number.isFinite(longitude)) {
        normalized.longitude = longitude;
      }
  ...
  const combinedParcelCandidates = [
  ...
  for (const parcelCandidate of combinedParcelCandidates) {
    const parsed =
      parseGridFromPcn(parcelCandidate) ||
      deriveGridPartsFromPcn(parcelCandidate);
    if (!parsed || typeof parsed !== "object") continue;
    for (const [field, value] of Object.entries(parsed)) {
      if (!COUNTY_ADDRESS_ENSURE_FIELDS.includes(field)) continue;
      setFieldIfMeaningful(field, value);
    }
  }
  ...
  if (resolvedRaw) {
    enrichAddressFromUnnormalized(normalized, resolvedRaw);
  }

  setFieldIfMeaningful("county_name", defaultCountyName);
  setFieldIfMeaningful("state_code", defaultStateCode);
  setFieldIfMeaningful("country_code", defaultCountryCode);

  if (
    hasMeaningful("state_code") &&
    !hasMeaningful("country_code")
  ) {
    normalized.country_code = defaultCountryCode || "US";
  }

  if (!hasMeaningful("postal_code")) {
    normalized.postal_code = null;
    normalized.plus_four_postal_code = null;
  } else if (!hasMeaningful("plus_four_postal_code")) {
    normalized.plus_four_postal_code = null;
  }

  const GRID_LENGTHS = { township: 2, range: 2, section: 2, block: 3, lot: 4 };
  for (const [field, length] of Object.entries(GRID_LENGTHS)) {
    if (!hasMeaningful(field)) continue;
    normalized[field] = padGridValue(normalized[field], length);
  }

  const latitude = parseCoordinate(normalized.latitude);
  const longitude = parseCoordinate(normalized.longitude);
  normalized.latitude = Number.isFinite(latitude) ? latitude : null;
  normalized.longitude = Number.isFinite(longitude) ? longitude : null;
  if (
    (normalized.latitude == null) !== (normalized.longitude == null)
  ) {
    normalized.latitude = null;
  ...
  const pushRequestIdCandidate = (candidate) => {
    ...
    if (Array.isArray(candidate)) {
      for (const entry of candidate) {
        pushRequestIdCandidate(entry);
      }
      return;
    }
    if (typeof candidate === "object") {
      pushRequestIdCandidate(candidate.request_identifier);
      pushRequestIdCandidate(candidate.requestIdentifier);
      return;
    }
    const str = String(candidate).trim();
    if (str.length) {
      requestIdQueue.push(str);
    }
  };
  pushRequestIdCandidate(payload.request_identifier);
  if (Array.isArray(requestIdentifierCandidates)) {
    for (const candidate of requestIdentifierCandidates) {
      pushRequestIdCandidate(candidate);
    }
  }
  if (
    ADDRESS_FALLBACK_CONTEXT &&
  ...
  for (const source of combinedFieldSources) {
    if (!source || typeof source !== "object") continue;
    pushRequestIdCandidate(source.request_identifier);
  }
  const resolvedRequestIdentifier =
    requestIdQueue.length > 0 ? requestIdQueue[0] : null;

  const sourceCandidateQueue = [];
  const pushSourceCandidate = (candidate) => {
    if (!candidate) return;
    if (Array.isArray(candidate)) {
      for (const entry of candidate) {
        pushSourceCandidate(entry);
      }
      return;
    }
    if (candidate && typeof candidate === "object") {
      sourceCandidateQueue.push(candidate);
    }
  };
  pushSourceCandidate(payload.source_http_request);
  if (Array.isArray(sourceHttpRequestCandidates)) {
  ...
  if (
    ...
    for (const candidate of ADDRESS_FALLBACK_CONTEXT.sourceHttpRequestCandidates) {
      pushSourceCandidate(candidate);
    }
  }
  for (const source of combinedFieldSources) {
    if (!source || typeof source !== "object") continue;
    pushSourceCandidate(source.source_http_request);
  }
  const resolvedSourceRequest = sourceCandidateQueue.length
    ? resolveSourceHttpRequestCandidate(...sourceCandidateQueue)
    : null;
  const preparedSource = resolvedSourceRequest
    ? prepareSourceHttpRequest(resolvedSourceRequest)
    : null;

  const attachRequestAndSource = (target) => {
    if (resolvedRequestIdentifier) {
      target.request_identifier = resolvedRequestIdentifier;
    } else if (
  ...
  const persistAddress = (candidate) => {
    if (!candidate || typeof candidate !== "object") {
      fs.writeFileSync(addressFilePath, JSON.stringify(candidate, null, 2));
      return;
    }

    let payloadToWrite = sanitizeAddressPayloadForWrite(candidate);
    if (payloadToWrite && typeof payloadToWrite === "object") {
      payloadToWrite =
        ensureAddressOutputFieldPresence(payloadToWrite) || payloadToWrite;
    } else {
      payloadToWrite = { ...candidate };
      payloadToWrite =
        ensureAddressOutputFieldPresence(payloadToWrite) || payloadToWrite;
    }

    if (payloadToWrite && typeof payloadToWrite === "object") {
      attachRequestAndSource(payloadToWrite);
    }
  ...
  if (normalizedCoverage) {
    ...
    persistAddress(normalizedOutput);
    return;
  }

  const fallbackRaw =
    resolvedRaw ||
    composeFallbackUnnormalizedAddressFromFields(normalized) ||
    null;

  if (!fallbackRaw) {
    const normalizedFallback = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
    for (const field of NORMALIZED_ADDRESS_FIELDS) {
    ...
    if (!normalizedFallback.postal_code) {
      normalizedFallback.plus_four_postal_code = null;
    }
    if (
      normalizedFallback.state_code &&
      !normalizedFallback.country_code
    ) {
      normalizedFallback.country_code = defaultCountryCode || "US";
    }
    persistAddress(normalizedFallback);
    return;
  }
...
async function main() {
  ...
  finalizeCountyAddressSchemaSurface(addressOutputPath);
  enforceCountyAddressFinalSurface(addressOutputPath, {
    unnormalizedCandidates: fallbackUnnormalizedValues,
    fieldSources: [
      ...finalFieldSources,
      ...(solidifyOptions.fieldSources || []),
    ],
    fieldFallbacks: {
      ...(solidifyOptions.fieldFallbacks || {}),
    },
    coordinateCandidates: solidifyOptions.coordinateCandidates,
    requestIdentifierCandidates: solidifyOptions.requestIdentifierCandidates,
    sourceHttpRequestCandidates: solidifyOptions.sourceHttpRequestCandidates,
  ...
  ensureRawAddressFinalFieldCoverage(finalAddressPath);
  enforceRawVariantWhenNormalizedIncomplete(finalAddressPath, {
    unnormalizedCandidates: [
      ...(Array.isArray(finalUnnormalizedCandidates)
        ? finalUnnormalizedCandidates
        : []),
  ...
  enforceFinalAddressOneOfOutput(finalAddressPath);
  finalizeCountyAddressNormalization(finalAddressPath, {
    rawCandidates: [
      ...(Array.isArray(finalUnnormalizedCandidates)
        ? finalUnnormalizedCandidates
        : []),
    ...
    plus4Candidates: [
      ...
      plus4,
      safeParsedUnnormalizedCityState &&
        safeParsedUnnormalizedCityState.plus4,
    ],
    countryCandidates: ["US"],
    defaultCountyName:
      formattedCountyName ||
  ...
  finalizeCountyAddressVariantSimple(finalAddressPath, {
    unnormalizedCandidates: [
      ...(Array.isArray(finalUnnormalizedCandidates)
        ? finalUnnormalizedCandidates
        : []),
  ...
  enforceCountyAddressNormalizedVariant(finalAddressPath, {
    unnormalizedCandidates: [
      ...(Array.isArray(finalUnnormalizedCandidates)
        ? finalUnnormalizedCandidates
        : []),

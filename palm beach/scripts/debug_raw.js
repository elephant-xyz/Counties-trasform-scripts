...
// Skip writing auto-generated relationship files locally. Downstream systems
// populate these UR-based links, so keep them absent/null in this transform.
fs.writeFileSync = function patchedWriteFileSync(targetPath, data, ...args) {
  try {
    ...
    // Fall through to the original writer if detection fails.
  }
...
function clampAddressToSingleBranchStrict(addressPath) {
  ...
  // Emit the raw branch with the full schema surface populated (nullable) so
  // oneOf validation never fails due to missing fields.
  ...
  RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
    ...
    if (field === "source_http_request") {
      rawOut[field] = sourceHttp || null;
      return;
    }
    let value = snapshot[field];
    if (field === "county_name" && countyName !== undefined) {
      value = countyName;
    }
    if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
      const numeric = parseCoordinate(value);
...
fs.writeFileSync = function patchedWriteFileSync(targetPath, data, ...args) {
  ...
  // Null out address relationships on any JSON payload so downstream can safely
  // populate URIs without local stubs.
  if (
    typeof targetPath === "string" &&
...
function finalizeCountyAddressOutput(addressPath, options = {}) {
  ...
  if (resolvedRaw) {
    ...
    const rawOut =
      buildRawAddressMinimalSurface(rawSource, resolvedRaw) ||
...
// Final resolver for the address oneOf: prefer a complete normalized payload
// when available, otherwise emit a raw branch anchored on unnormalized_address
// while still surfacing every schema field as null so required keys exist.
function finalizeAddressOneOfBranch(addressPath) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  const snapshot = readJSONIfExists(addressPath);
  if (!snapshot || typeof snapshot !== "object" || Array.isArray(snapshot)) {
    removeFileIfExists(addressPath);
  ...
  const rawCandidate = safeNullIfEmpty(snapshot.unnormalized_address);
  const normalizedSurface =
    typeof ensureNormalizedAddressSchemaSurface === "function"
      ? ensureNormalizedAddressSchemaSurface({ ...snapshot })
      : null;
    ...
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      let value =
        normalizedSurface[field] !== undefined
          ...
          : snapshot[field];
      if (field === "request_identifier") {
        value = snapshot.request_identifier === undefined ? null : snapshot.request_identifier;
      } else if (field === "source_http_request") {
        value =
          prepareSourceHttpRequest(
            snapshot.source_http_request ?? normalizedSurface.source_http_request,
          ) || null;
  ...
  RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
    ...
    if (field === "request_identifier") {
      rawOut[field] =
        snapshot.request_identifier === undefined ? null : snapshot.request_identifier;
      return;
    }
    if (field === "source_http_request") {
      rawOut[field] = prepareSourceHttpRequest(snapshot.source_http_request) || null;
      return;
    }
    let value = snapshot[field];
...
function buildRawAddressMinimalSurface(sourcePayload, rawValue) {
  ...
  if (
    Object.prototype.hasOwnProperty.call(result, "latitude") &&
    Object.prototype.hasOwnProperty.call(result, "longitude") &&
...
function enforceAddressSchemaSurfaceForOutput(addressPath) {
  if (!addressPath || !fs.existsSync(addressPath)) {
    return;
  }
  ...
  const rawValue =
    typeof payload.unnormalized_address === "string"
      ? payload.unnormalized_address.trim()
      : "";
  const hasRawVariant = rawValue.length > 0;
  ...
  fieldList.forEach((field) => {
    if (!hasRawVariant && field === "unnormalized_address") {
      return;
    }
    if (field === "unnormalized_address") {
      out[field] = rawValue;
    ...
    if (field === "source_http_request") {
      out[field] = preparedSource ? deepClone(preparedSource) : null;
      return;
    }
    if (field === "request_identifier") {
      const identifier = safeNullIfEmpty(payload.request_identifier);
      out[field] =
        identifier === undefined || identifier === null ? null : identifier;
      return;
    }
    let value = payload[field];
    if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
  ...
  if (!hasRawVariant && Object.prototype.hasOwnProperty.call(out, "unnormalized_address")) {
...
// Final guard to ensure address.json cleanly selects a single oneOf branch:
// - If normalized coverage is complete, emit only normalized fields.
// - Otherwise, prefer the raw branch anchored on the unnormalized string and
//   populate the full nullable surface so required keys are present.
...
// Final guard to collapse address.json into a single valid oneOf branch.
// - If the normalized surface is fully populated, emit only normalized fields.
// - Otherwise prefer the raw branch and hydrate every allowed field (nullable)
//   so required keys are present even when the source only supplies the
//   unnormalized string.
function enforceTerminalAddressOneOfSurface(addressPath) {
  ...
  const rawOut =
    ensureRawAddressRequiredCoverage(payload, rawValue, RAW_ONE_OF_ALLOWED_FIELDS) ||
    null;
  if (!rawOut) {
    removeFileIfExists(addressPath);
    return;
...
// Final reducer: pick a single valid address branch and guarantee the full
// schema surface is present (nullable) so oneOf validation never complains
// about missing normalized fields when we only have an unnormalized string.
function stabilizeFinalAddressBranch(addressPath, defaults = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  ...
  const fallbackCounty =
    safeNullIfEmpty(payload.county_name) ||
    safeNullIfEmpty(defaults.county) ||
    null;
  const fallbackState =
    safeNullIfEmpty(payload.state_code) || safeNullIfEmpty(defaults.state) || null;
  ...
  if (rawValue) {
    ...
    RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
      ...
      if (field === "source_http_request") {
        const prepared = prepareSourceHttpRequest(
          resolveSourceHttpRequest(payload.source_http_request),
        );
        finalOut.source_http_request = prepared ? prepared : null;
        return;
      }
      ...
      if (typeof value === "string") {
        const trimmed = value.trim();
        finalOut[field] = trimmed.length ? trimmed : null;
        return;
      }
      finalOut[field] = value === undefined ? null : value;
    });
  ...
  if (
    ...
    finalOut.country_code = fallbackCountry || "US";
  }
...
// Final oneOf guard: when we have an unnormalized address from the source,
// rebuild the payload on the raw branch using the full schema surface so all
// required keys are present (as null) and the normalized branch is not selected.
function lockRawAddressOneOf(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  ...
  const fallbackCounty =
    safeNullIfEmpty(snapshot.county_name) ||
    safeNullIfEmpty(options.county_name) ||
    safeNullIfEmpty(options.county) ||
    null;
  const fallbackState =
    safeNullIfEmpty(snapshot.state_code) ||
    safeNullIfEmpty(options.state_code) ||
  ...
  if (rawValue) {
    ...
    RAW_ONE_OF_ALLOWED_FIELDS.forEach((field) => {
      ...
      if (field === "request_identifier") {
        rawOut.request_identifier = resolvedRequestId;
        return;
      }
      if (field === "source_http_request") {
        rawOut.source_http_request = resolvedSourceHttp;
    ...
    if (
      ...
      rawOut.country_code = fallbackCountry || "US";
    }
    writeJSON(addressPath, rawOut);
    return;
  }
  ...
  if (normalizedReady) {
    ...
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      if (field === "request_identifier") {
        normalizedOut.request_identifier = resolvedRequestId;
        return;
      }
      if (field === "source_http_request") {
    ...
    if (
      ...
      normalizedOut.country_code = fallbackCountry || "US";
    }
    if (
      Object.prototype.hasOwnProperty.call(normalizedOut, "unnormalized_address")
    ) {
...
// unnormalized string, emit the raw branch but keep the full schema surface
// populated (nullable) so required fields like latitude/longitude/street_*
// remain present. When a complete normalized surface exists, drop the raw
// field and emit the normalized branch instead.
function coerceAddressToSingleBranch(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  ...
  if (rawValue && !normalizedReady) {
    ...
    RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
      ...
      if (field === "source_http_request") {
        const prepared = prepareSourceHttpRequest(value);
        hydrated[field] = prepared ? deepClone(prepared) : null;
        return;
      }
      if (typeof value === "string") {
        const trimmed = value.trim();
    ...
    if (
      ...
      hydrated.country_code = defaultCountry;
    }
    writeJSON(addressPath, hydrated);
    return;
  }
...
// Force the final address payload to align with a single oneOf branch. Prefer
// the raw branch when an unnormalized_address is present; otherwise emit the
// normalized surface. Always include the full schema field surface (nullable)
// so required keys like latitude/longitude/street_* are present, preventing
// validation errors about missing properties.
function enforceAddressTemplateCoverage(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  const payload = readJSONIfExists(addressPath);
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    removeFileIfExists(addressPath);
  ...
  const rawValue = safeNullIfEmpty(payload.unnormalized_address);
  const defaultCounty = options.defaultCounty || null;
  const defaultState = options.defaultState || null;
  const defaultCountry = options.defaultCountry || "US";
  const preparedSource = prepareSourceHttpRequest(payload.source_http_request);
  const requestIdentifier =
    payload.request_identifier === undefined ? null : payload.request_identifier;
  ...
  if (rawValue && !hasNormalized) {
    ...
    const rawSeed = {
      ...
      county_name: payload.county_name || defaultCounty || null,
      state_code: payload.state_code || defaultState || null,
      country_code:
        payload.country_code ||
        (payload.state_code || defaultState ? defaultCountry : null),
      ...
      plus_four_postal_code: payload.postal_code
        ? payload.plus_four_postal_code ?? null
        : null,
  ...
  // Normalized branch: keep the full schema surface and drop the raw field so
  // the oneOf resolves cleanly.
  const normalizedOut = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
  NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
    if (field === "request_identifier") {
      normalizedOut[field] = safeNullIfEmpty(payload.request_identifier) ?? null;
      return;
    }
    if (field === "source_http_request") {
      normalizedOut[field] = preparedSource ? deepClone(preparedSource) : null;
      return;
    }
    let value = normalizedSurface[field];
    if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
  ...
  if (!normalizedOut.county_name && defaultCounty) {
    normalizedOut.county_name = defaultCounty;
  }
  if (!normalizedOut.state_code && defaultState) {
    normalizedOut.state_code = defaultState;
  }
...
function buildFinalRawAddressOutput(rawValue, options = {}) {
  const trimmedRaw = safeNullIfEmpty(rawValue);
  if (!trimmedRaw) return null;

  const seed =
    options.seed && typeof options.seed === "object" && !Array.isArray(options.seed)
      ? options.seed
      : {};
  const rawOut = { ...RAW_ONE_OF_SCHEMA_TEMPLATE };
  RAW_ONE_OF_ALLOWED_FIELDS.forEach((field) => {
  ...
  if (requestId !== undefined) {
    rawOut.request_identifier = requestId ?? null;
  }
  ...
  const countyName =
    options.countyName === undefined ? seed.county_name : options.countyName;
  rawOut.county_name = safeNullIfEmpty(countyName);

  const countryCode =
    options.countryCode === undefined ? seed.country_code : options.countryCode;
  rawOut.country_code = safeNullIfEmpty(countryCode);

  RAW_ONE_OF_ALLOWED_FIELDS.forEach((field) => {
    if (field === "unnormalized_address") return;
    if (rawOut[field] !== null) return;
    if (!Object.prototype.hasOwnProperty.call(seed, field)) return;

    if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
      const numeric = parseCoordinate(seed[field]);
      rawOut[field] = Number.isFinite(numeric) ? numeric : null;
      return;
    }

    rawOut[field] = sanitizeAddressFieldValue(field, seed[field]);
  });

  const latitudeCandidate =
    options.latitude === undefined ? seed.latitude : options.latitude;
  const longitudeCandidate =
    options.longitude === undefined ? seed.longitude : options.longitude;
  const lat = parseCoordinate(latitudeCandidate);
  const lon = parseCoordinate(longitudeCandidate);
  if (Number.isFinite(lat) && Number.isFinite(lon)) {
    rawOut.latitude = lat;
...
// Clamp the final address payload to a single oneOf branch. Prefer a complete
// normalized address; otherwise emit a lean raw branch that only carries the
// unnormalized string plus request/source metadata so it matches the raw
// schema without extra normalized fields.
function enforceLeanCountyAddress(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  ...
  if (normalizedComplete && !preferRaw) {
    ...
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      ...
      if (field === "source_http_request") {
        normalizedOut[field] = sourceHttp ? deepClone(sourceHttp) : null;
        return;
      }
...
// (nullable) so required keys are always present.
  ...
  if (rawValue) {
    ...
    RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
      ...
      if (field === "request_identifier") {
        ...
        rawOut.request_identifier =
          requestId === undefined ? null : requestId || null;
        return;
      }
      ...
      const candidateValue =
        snapshot[field] !== undefined
          ...
          ? normalizedSurface[field]
          : null;
      if (!candidateValue && field === "county_name" && options.countyFallback) {
      ...
      rawOut[field] = sanitizeAddressFieldValue(field, candidateValue);
    });
...
// Final clamp to emit a single valid oneOf branch. If the normalized surface is
// fully populated, emit only the normalized fields (drop unnormalized_address).
// Otherwise, emit a lean raw payload anchored on the unnormalized string with
// just the metadata fields the schema permits.
function finalizeMinimalAddressBranch(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  const snapshot = readJSONIfExists(addressPath);
  if (!snapshot || typeof snapshot !== "object" || Array.isArray(snapshot)) {
    removeFileIfExists(addressPath);
  ...
  const preparedSource = prepareSourceHttpRequest(sourceHttp);
  const countyName = safeNullIfEmpty(
    resolveFirstNonEmptyString([
      snapshot.county_name,
      ...(options.countyNameCandidates || []),
    ]),
  );
  ...
  if (normalizedComplete) {
    ...
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      ...
      const value = sanitizeAddressFieldValue(field, normalizedSurface[field]);
      normalizedOut[field] = value === undefined ? null : value;
    });
    if (!normalizedOut.postal_code) {
      normalizedOut.plus_four_postal_code = null;
  ...
  if (hasUnnormalized) {
    return (
      buildStrictRawOneOfPayload({
        ...cloned,
        unnormalized_address: trimmedUnnormalized,
      }) || buildRawAddressMinimalSurface(cloned, trimmedUnnormalized)
    );
  }
...
function enforceRawPreferenceWhenAvailable(addressPath, options = {}) {
  ...
  if (rawValue && (preferRaw || !normalizedReady)) {
    // Stick to the raw branch when we have an unnormalized source string by
    // rebuilding the payload on the raw surface and stripping normalized-only
    // values. Preserve only a minimal context (jurisdiction, postal hints,
    // request identifiers, coordinates if present) so oneOf validation
    // doesn't see a half-populated normalized branch.
    const preserve = new Set(
      options.preserveFields || RAW_ONE_OF_MINIMAL_FIELDS,
    );
    const baseRaw = { ...RAW_ADDRESS_SCHEMA_TEMPLATE };
    baseRaw.unnormalized_address = rawValue;
    baseRaw.request_identifier =
      requestIdentifier === undefined ? null : requestIdentifier;
    baseRaw.source_http_request = sourceHttp || payload.source_http_request || null;
    ...
    RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
      ...
      if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
        const numeric = parseCoordinate(value);
        baseRaw[field] = Number.isFinite(numeric) ? numeric : null;
        return;
      }

      if (typeof value === "string") {
        const trimmed = value.trim();
        baseRaw[field] = trimmed.length ? trimmed : null;
        return;
      }

      baseRaw[field] = value === undefined ? null : value;
    });
...
function finalizeAddressBranchSelection(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  ...
  if (rawValue) {
    ...
    RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
      if (field === "unnormalized_address") return;
      if (field === "request_identifier") {
        rawOut.request_identifier = resolvedRequestIdentifier ?? null;
        return;
      }
      if (field === "source_http_request") {
        rawOut.source_http_request = resolvedSourceHttp;
    ...
    if (!rawOut.county_name && options.fallbackCounty) {
      rawOut.county_name = options.fallbackCounty;
    }
    if (!rawOut.state_code && options.fallbackState) {
      rawOut.state_code = options.fallbackState;
    }
    if (!rawOut.postal_code) {
      rawOut.plus_four_postal_code = null;
    }
    ...
    if (
      ...
      rawOut.country_code = options.fallbackCountry || "US";
    }
  ...
  if (normalizedReady) {
    ...
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      ...
      if (field === "source_http_request") {
        normalizedOut[field] = resolvedSourceHttp;
        return;
      }
      if (field === "request_identifier") {
        normalizedOut[field] =
          resolvedRequestIdentifier === undefined
            ? safeNullIfEmpty(normalizedSurface[field])
            : resolvedRequestIdentifier;
        return;
      }
      const value = normalizedSurface[field];
    ...
    if (
      ...
      normalizedOut.country_code = options.fallbackCountry || "US";
    }
    if (
      Object.prototype.hasOwnProperty.call(
        normalizedOut,
...
const NORMALIZED_CORE_COUNTY_FIELDS = Object.freeze([
  "street_number",
  "street_name",
  "city_name",
  "state_code",
...
function lockFinalCountyAddressBranch(addressPath) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  const snapshot = readJSONIfExists(addressPath);
  if (!snapshot || typeof snapshot !== "object" || Array.isArray(snapshot)) {
    removeFileIfExists(addressPath);
  ...
  NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
    let value = snapshot[field];
    if (field === "source_http_request") {
      normalizedCandidate[field] = prepareSourceHttpRequest(value) || null;
      return;
    }
    if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
      const numeric = parseCoordinate(value);
  ...
  const hasNormalizedCoverage = NORMALIZED_CORE_COUNTY_FIELDS.every((field) => {
    const value = normalizedCandidate[field];
    if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
      return Number.isFinite(value);
    }
    return hasMeaningfulAddressValue(value);
  });
  ...
  if (hasNormalizedCoverage) {
    const normalizedOut = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      normalizedOut[field] = normalizedCandidate[field];
    });
    if (!normalizedOut.postal_code) {
      normalizedOut.plus_four_postal_code = null;
    }
...
// Final guard to collapse the address into a single valid oneOf branch. Prefer the
// normalized branch only when coverage is complete; otherwise emit the raw branch
// with the full schema surface (nullable) so required properties are always present.
function enforceFinalCountyAddressOneOf(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  ...
  const fallbackCounty = safeNullIfEmpty(
    options.fallbackCountyName ||
      payload.county_name ||
      payload.county_jurisdiction,
  );
  ...
  if (!rawOut.county_name && fallbackCounty) {
    rawOut.county_name = fallbackCounty;
  }
  if (
    hasMeaningfulAddressValue(rawOut.state_code) &&
    !hasMeaningfulAddressValue(rawOut.country_code)
...
// Final guard to lock the address to a single schema branch. Prefer normalized
// when it is complete; otherwise emit the raw branch anchored on the
// unnormalized string with all schema fields present (nullable) so required
// keys never go missing.
function finalizeAddressBranchForOutput(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  ...
  if (normalizedReady && !preferRawBranch) {
    ...
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      let value =
        payload[field] !== undefined ? payload[field] : normalizedSurface[field];
      if (field === "request_identifier") {
        value =
          options.requestIdentifier !== undefined
            ? safeNullIfEmpty(options.requestIdentifier)
  ...
  RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
    ...
    if (field === "request_identifier") {
      rawOut[field] =
        options.requestIdentifier !== undefined
          ? safeNullIfEmpty(options.requestIdentifier) ?? null
          : safeNullIfEmpty(payload.request_identifier) ?? null;
      return;
    }
    if (field === "source_http_request") {
      rawOut[field] =
        prepareSourceHttpRequest(
          payload.source_http_request ?? options.sourceHttpRequest,
        ) || null;
      return;
    }
  ...
  if (options.countyFallback && !hasMeaningfulAddressValue(rawOut.county_name)) {
    rawOut.county_name = safeNullIfEmpty(options.countyFallback) || null;
  }
  if (options.stateFallback && !hasMeaningfulAddressValue(rawOut.state_code)) {
...
// Keep the final address payload aligned to a single schema branch with every
// allowed field present (nullable) so oneOf validation does not complain about
// missing required keys. Prefer normalized only when the coverage is complete;
// otherwise emit the raw branch anchored by the unnormalized string.
function alignAddressOneOfSurfaceForOutput(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  ...
  const rawOut = { ...RAW_ONE_OF_SCHEMA_TEMPLATE };
  RAW_ONE_OF_ALLOWED_FIELDS.forEach((field) => {
    rawOut[field] = field === "unnormalized_address" ? rawValue : null;
  });
  ...
  rawOut.source_http_request = sourceHttp;
...
// Final guardrail: prefer the raw branch when we only have an unnormalized
// string, and emit the normalized branch only when the strict normalized
// surface (coords + core strings) is present. Always populate the full schema
// surface (nullable) so oneOf validation never flags missing required keys.
function finalizeAddressBranchForSchema(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  ...
  const normalizedSurface =
    typeof ensureNormalizedAddressSchemaSurface === "function"
      ...
      : { ...snapshot };
  const normalizedReady =
    normalizedSurface && hasNormalizedStrictCoverage({ ...normalizedSurface });

  if (normalizedReady && !options.preferRaw) {
    const normalizedOut = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      let value = normalizedSurface[field];
      if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
  ...
  RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
    ...
    rawOut[field] = value === undefined ? rawOut[field] : value;
  });
...
// Final safeguard to force a single address branch. Prefer the raw oneOf branch
// whenever an unnormalized string exists; otherwise emit a fully covered
// normalized payload. Always keep the full schema surface so required keys are
// present even when values are null.
function finalizeRawOrNormalizedAddress(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  ...
  if (rawValue && (!normalizedComplete || options.preferRaw)) {
    const rawOut = { ...RAW_ONE_OF_SCHEMA_TEMPLATE };
    RAW_ONE_OF_ALLOWED_FIELDS.forEach((field) => {
      if (field === "unnormalized_address") {
        rawOut.unnormalized_address = rawValue;
      ...
      if (field === "request_identifier") {
        value =
          value === undefined || value === null ? null : safeNullIfEmpty(value);
      } else if (field === "source_http_request") {
        value = prepareSourceHttpRequest(value) || null;
      } else if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
        const numeric = parseCoordinate(value);
...
function coerceAddressToSingleSchemaBranch(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  const snapshot = readJSONIfExists(addressPath);
  if (!snapshot || typeof snapshot !== "object" || Array.isArray(snapshot)) {
    removeFileIfExists(addressPath);
  ...
  const preferRawWhenAvailable =
    options.preferRawWhenAvailable === undefined
      ? true
      : !!options.preferRawWhenAvailable;
  const rawValue = safeNullIfEmpty(snapshot.unnormalized_address);
  const requestId = safeNullIfEmpty(snapshot.request_identifier);
  const sourceHttp = resolveSourceHttpRequest(snapshot.source_http_request);
  ...
  const shouldUseNormalized =
    normalizedComplete && (!preferRawWhenAvailable || !rawValue);
  ...
  if (normalizedSurface) {
    const normalizedOut = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      normalizedOut[field] = sanitizeField(field, normalizedSurface[field]);
    });
    if (!normalizedOut.postal_code) {
      normalizedOut.plus_four_postal_code = null;
    }
...
// Final guard to pick exactly one address schema branch: prefer normalized when
// fully populated, otherwise emit the raw branch anchored on the unnormalized
// string while keeping the full schema surface nullable.
  ...
  if (normalizedComplete) {
    ...
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      let value =
        payload[field] !== undefined ? payload[field] : normalizedSurface[field];
      if (field === "request_identifier") {
        value = resolvedRequestIdentifier === undefined ? null : resolvedRequestIdentifier;
      } else if (field === "source_http_request") {
        value = prepareSourceHttpRequest(resolvedSourceHttp) || null;
      } else if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
        const numeric = parseCoordinate(value);
...
// Final safety net: pick a single oneOf branch and ensure every schema field is
// present (nullable) so validation never sees missing required keys. Prefer a
// normalized branch only when the coverage is complete; otherwise emit the raw
// branch anchored on the unnormalized string with the full schema surface.
function enforceAddressOneOfSurfaceCompliance(addressPath) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  ...
  RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
    ...
    if (field === "request_identifier") {
      rawOut[field] = resolvedRequestIdentifier === undefined ? null : resolvedRequestIdentifier;
      return;
    }
    if (field === "source_http_request") {
      rawOut[field] = resolvedSourceHttp;
      return;
    }
    let value = payload[field];
    if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
...
// When we have complete normalized coverage, force the final output to the
// normalized branch and drop the raw string to satisfy the address oneOf.
function preferNormalizedCountyAddressOutput(addressPath) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
...
// Final guardrail to emit a schema-compliant address. Prefer the fully
// normalized branch when coverage is complete; otherwise emit the raw branch
// with the full schema surface (nullable) so oneOf validation sees every
// required key instead of missing fields.
function enforceCanonicalCountyAddress(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  ...
  if (normalizedComplete) {
    ...
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      let value =
        payload[field] !== undefined ? payload[field] : normalizedSurface[field];
      if (field === "request_identifier") {
        value = resolvedRequestId === undefined ? null : resolvedRequestId;
      } else if (field === "source_http_request") {
        value = prepareSourceHttpRequest(resolvedSourceHttp) || null;
      } else if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
        const numeric = parseCoordinate(value);
...
// Final guardrail to pick exactly one address oneOf branch. Prefer normalized
// when coverage is complete; otherwise emit the raw branch anchored on the
// unnormalized string while keeping the full schema surface (nullable) so the
// validator never reports missing required fields.
function finalizeAddressOneOfSelection(addressPath, options = {}) {
  if (!addressPath) return;

  const existingPayload = fs.existsSync(addressPath)
    ? readJSONIfExists(addressPath) || {}
    : {};
  const normalizedSurface =
    typeof ensureNormalizedAddressSchemaSurface === "function"
  ...
  if (normalizedReady) {
    ...
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      ...
      if (field === "request_identifier") {
        value =
          requestIdentifier === undefined ? normalizedSurface[field] : requestIdentifier;
      } else if (field === "source_http_request") {
        value = prepareSourceHttpRequest(resolvedSourceHttp) || null;
      } else if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
        const numeric = parseCoordinate(value);
  ...
  RAW_ONE_OF_ALLOWED_FIELDS.forEach((field) => {
    ...
    if (field === "request_identifier") {
      rawOut[field] =
        requestIdentifier === undefined
          ? safeNullIfEmpty(existingPayload.request_identifier)
          : requestIdentifier;
      return;
    }
    if (field === "source_http_request") {
    ...
    let value = existingPayload[field];
    if (value === undefined && normalizedSurface) {
      value = normalizedSurface[field];
    }
    if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
...
// so required keys are never missing.
function hardenFinalAddressOneOf(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  ...
  if (normalizedReady) {
    ...
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      ...
      if (field === "request_identifier") {
        ...
        value = preparedSourceHttp ? deepClone(preparedSourceHttp) : null;
      } else if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
        const numeric = parseCoordinate(value);
        value = Number.isFinite(numeric) ? numeric : null;
      } else if (typeof value === "string") {
  ...
  if (rawValue) {
    ...
    RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
      ...
      if (field === "source_http_request") {
        rawOut[field] = preparedSourceHttp ? deepClone(preparedSourceHttp) : null;
        return;
      }
      let value = payload[field];
      if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
...
// Simplified final guard: pick exactly one address oneOf branch. Prefer the raw
// unnormalized branch whenever we have a source string; only emit normalized
// when all required normalized fields are present.
function enforceFinalCountyAddressOneOfSimple(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  ...
  if (normalizedReady) {
    ...
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      ...
      if (field === "request_identifier") {
        value = requestIdentifier === undefined ? value : requestIdentifier;
      } else if (field === "source_http_request") {
        value = preparedSourceHttp ? deepClone(preparedSourceHttp) : null;
      } else if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
        const numeric = parseCoordinate(value);
  ...
  if (rawValue) {
    ...
    if (!rawOut.state_code && options.stateFallback) {
      rawOut.state_code = safeNullIfEmpty(options.stateFallback) || null;
    }
    if (!rawOut.postal_code) {
      rawOut.plus_four_postal_code = null;
    }
...
// Final canonicalizer: pick exactly one address oneOf branch. Use normalized
// only when it is fully populated; otherwise emit the raw branch anchored on
// the unnormalized string with the full schema surface present (nullable) so
// required keys are never missing.
function commitCanonicalAddressOneOf(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;

  const payload = readJSONIfExists(addressPath) || {};
  const rawCandidates = Array.isArray(options.rawCandidates)
    ? options.rawCandidates.filter((value) => value !== undefined && value !== null)
    : [];
  const rawValue = safeNullIfEmpty(
    resolveFirstNonEmptyString([payload.unnormalized_address, ...rawCandidates]),
  );
  ...
  const resolvedRequestId =
    options.requestIdentifier !== undefined
      ? safeNullIfEmpty(options.requestIdentifier)
      : safeNullIfEmpty(payload.request_identifier);
  const sourceHttpCandidates = Array.isArray(options.sourceHttpRequestCandidates)
    ? options.sourceHttpRequestCandidates
    : [];
  const resolvedSourceHttp =
    prepareSourceHttpRequest(
      resolveSourceHttpRequest(payload.source_http_request, ...sourceHttpCandidates),
    ) || null;
  const defaultCountry = options.defaultCountryCode || "US";
  const preferRawWhenAvailable =
    options.preferRawWhenAvailable === true && Boolean(rawValue);

  if (normalizedReady && !preferRawWhenAvailable) {
    const normalizedOut = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      let value =
        payload[field] !== undefined ? payload[field] : normalizedSurface[field];
      if (field === "request_identifier") {
        value = resolvedRequestId === undefined ? value : resolvedRequestId;
      } else if (field === "source_http_request") {
        value = resolvedSourceHttp ? deepClone(resolvedSourceHttp) : null;
      } else if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
        const numeric = parseCoordinate(value);
        value = Number.isFinite(numeric) ? numeric : null;
      } else if (typeof value === "string") {
  ...
  if (!hasMeaningfulAddressValue(rawOut.county_name) && options.defaultCountyName) {
    rawOut.county_name = titleCaseCounty(options.defaultCountyName);
  }
  if (!hasMeaningfulAddressValue(rawOut.state_code) && options.defaultStateCode) {
    rawOut.state_code = options.defaultStateCode;
  }
...
// Final clamp to honor source preference: use the unnormalized string when
// provided, otherwise emit a fully normalized surface. Always hydrate the full
// schema field set so oneOf validation sees required keys.
function enforceSourcePreferredAddressBranch(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  ...
  const fallbackState = safeNullIfEmpty(options.defaultStateCode);
  const fallbackCounty = safeNullIfEmpty(options.defaultCountyName);
  const defaultCountry = safeNullIfEmpty(options.defaultCountryCode) || "US";
  ...
  if (rawCandidate) {
    ...
    RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
      ...
      if (field === "state_code") {
        rawOut[field] = safeNullIfEmpty(snapshot.state_code) || fallbackState || null;
        return;
      }
      if (field === "county_name") {
        rawOut[field] = safeNullIfEmpty(snapshot.county_name) || fallbackCounty || null;
        return;
      }
...
// Final safeguard to keep address.json on exactly one schema branch.
// When an unnormalized string exists, emit the full raw schema surface with all
// fields present (nullable) to satisfy required keys. Otherwise emit a fully
// normalized payload only when the normalized fields are complete.
function clampCountyAddressOneOf(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  const snapshot = readJSONIfExists(addressPath);
  if (!snapshot || typeof snapshot !== "object" || Array.isArray(snapshot)) {
    removeFileIfExists(addressPath);
  ...
  if (rawValue) {
    ...
    RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
      ...
      if (field === "request_identifier") {
        rawOut[field] = resolveRequestIdentifier();
        return;
      }
      if (field === "source_http_request") {
        rawOut[field] = resolveSourceHttp();
        return;
      }
      let value = snapshot[field];
      if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
  ...
  if (normalizedComplete) {
    ...
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      let value =
        snapshot[field] !== undefined ? snapshot[field] : normalizedSurface[field];
      if (field === "request_identifier") {
        const resolved = resolveRequestIdentifier();
        value = resolved === undefined ? value : resolved;
      } else if (field === "source_http_request") {
        value = resolveSourceHttp();
      } else if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
        const numeric = parseCoordinate(value);
        value = Number.isFinite(numeric) ? numeric : null;
      } else if (typeof value === "string") {
...
// Final guardrail: rewrite the address payload onto a single valid oneOf branch
// with every schema field present (nullable) so validation doesn't complain
// about missing required properties. Prefer the raw branch when an
// unnormalized address is available; otherwise emit the normalized branch only
// when it is fully populated.
function finalizeCountyAddressPayload(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  const snapshot = readJSONIfExists(addressPath) || {};
  ...
  const rawCandidates = Array.isArray(options.rawCandidates)
    ? options.rawCandidates
    : [];
  rawCandidates.unshift(snapshot.unnormalized_address);
  const rawValue = safeNullIfEmpty(resolveFirstNonEmptyString(rawCandidates));
  ...
  if (rawValue) {
    ...
    RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
      ...
      if (field === "request_identifier") {
        output[field] = requestIdentifier === undefined ? null : requestIdentifier;
        return;
      }
      if (field === "source_http_request") {
        output[field] = sourceHttpRequest;
        return;
      }
      const fallback =
        options.defaults && options.defaults[field] !== undefined
          ? options.defaults[field]
          : undefined;
      const candidate = Object.prototype.hasOwnProperty.call(snapshot, field)
        ? snapshot[field]
        : fallback;
      output[field] = sanitizeValue(field, candidate);
    });
    if (!output.postal_code) {
      output.plus_four_postal_code = null;
    }
    ...
    if (
      ...
      output.country_code = options.defaults?.country_code || "US";
    }
  } else if (normalizedReady) {
    output = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      let value = Object.prototype.hasOwnProperty.call(snapshot, field)
        ? snapshot[field]
        : normalizedSurface[field];
      if (field === "request_identifier") {
        value = requestIdentifier === undefined ? value : requestIdentifier;
      } else if (field === "source_http_request") {
        value = sourceHttpRequest;
      }
      output[field] = sanitizeValue(field, value);
    });
    if (!output.postal_code) {
      output.plus_four_postal_code = null;
    }
...
// Final guard: prefer the raw/unnormalized branch when a raw string exists.
// Otherwise, emit the normalized branch only when it is fully populated.
function enforceRawPreferredAddressBranch(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  ...
  const requestIdentifier = safeNullIfEmpty(snapshot.request_identifier);
  const sourceHttp = prepareSourceHttpRequest(snapshot.source_http_request) || null;
  const defaultCounty = options.defaultCounty ?? null;
  const defaultState = options.defaultState ?? null;
  const defaultCountry = options.defaultCountry || "US";
  ...
  if (rawValue) {
    ...
    RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
      ...
      let value =
        snapshot[field] !== undefined ? snapshot[field] : normalizedSurface[field];
      if (field === "county_name" && defaultCounty && !hasMeaningfulAddressValue(value)) {
        value = defaultCounty;
      }
      if (field === "state_code" && defaultState && !hasMeaningfulAddressValue(value)) {
        value = defaultState;
      }
      if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
        const numeric = parseCoordinate(value);
...
// Final oneOf clamp for County: prefer the raw branch whenever an
// unnormalized string is present; otherwise emit the normalized branch only
// when complete. Ensures required fields stay present (nullable) so schema
// validation doesn't complain about missing properties.
function finalizeCountyOneOfBranch(addressPath, options = {}) {
  if (!addressPath || !fs.existsSync(addressPath)) return;
  ...
  const defaults = {
    county: options.defaultCounty ?? null,
  ...
  if (rawCandidate) {
    ...
    RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
      ...
      if (field === "request_identifier") {
        rawOut[field] = requestIdentifier;
        return;
      }
      if (field === "source_http_request") {
        rawOut[field] = sourceHttp;
    ...
    if (
      ...
      rawOut.country_code = defaults.country;
    }
    writeJSON(addressPath, rawOut);
    return;
  }
  ...
  if (normalizedReady) {
    ...
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      let value =
        snapshot[field] !== undefined ? snapshot[field] : normalizedSurface[field];
      if (field === "request_identifier") {
        value = requestIdentifier === null ? null : requestIdentifier;
      } else if (field === "source_http_request") {
        value = sourceHttp;
      } else if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
        const numeric = parseCoordinate(value);
        value = Number.isFinite(numeric) ? numeric : null;
      } else if (field === "county_name" && !hasMeaningfulAddressValue(value)) {
        value = defaults.county;
    ...
    if (
      ...
      normalizedOut.country_code = defaults.country;
    }
    if (
      Object.prototype.hasOwnProperty.call(normalizedOut, "unnormalized_address")
    ) {
...
async function main() {
  ...
  const inferredCounty = safeNullIfEmpty(
    resolveFirstNonEmptyString([
      existingAddress.county_name,
      unnormalizedSource.county_name,
      unnormalizedSource.county_jurisdiction,
  ...
  if (normalizedComplete) {
    ...
    if (inferredCounty !== undefined) {
      const trimmedCounty = safeNullIfEmpty(inferredCounty);
      if (trimmedCounty) {
        rawOut.county_name = trimmedCounty;
      }
    }

    const parsedLocality = parseCityStatePostal(resolvedRaw);
    const [rawStreetSegment] = resolvedRaw.split(",");
    const parsedStreet = parseLocationAddress(rawStreetSegment || resolvedRaw);
    const applyIfMissing = (field, candidate) => {
      if (candidate == null) return;
      if (hasMeaningfulAddressValue(rawOut[field])) return;
      if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
        const numeric = parseCoordinate(candidate);
        if (Number.isFinite(numeric)) rawOut[field] = numeric;
        return;
      }
      const sanitized = sanitizeAddressFieldValue(field, candidate);
  ...
  if (finalAddressOut) {
    ...
    if (
      (finalAddressOut.latitude == null) !==
      (finalAddressOut.longitude == null)
    ) {
      finalAddressOut.latitude = null;
      finalAddressOut.longitude = null;
    }
    ...
    writeJSON(addressOutputPath, finalAddressOut);
  } else {
  ...
  // Force a single, schema-compliant address branch so oneOf validation passes.
  ...
  enforceFinalAddressOneOfBranch(addressOutputPath, {
    rawCandidates,
    requestIdentifierCandidates: [
      resolvedRequestIdentifier,
      trimmedRequestIdentifier,
  ...
  enforceTerminalAddressOneOfSurface(addressOutputPath);
  stabilizeFinalAddressOneOf(addressOutputPath, {
    rawCandidates,
    requestIdentifier: resolvedRequestIdentifier,
    sourceHttpRequest: resolvedSourceHttp,
  });
  ...
  preferNormalizedCountyAddressOutput(addressOutputPath);
  enforceCanonicalCountyAddress(addressOutputPath, {
    rawCandidates,
    requestIdentifier: resolvedRequestIdentifier,
    sourceHttpRequest: resolvedSourceHttp,
  });
  ...
  // Final gate: if we cannot satisfy the normalized branch, emit a lean raw
  // payload anchored on the unnormalized string to keep oneOf validation happy.
  ...
  if (terminalNormalizedReady) {
    const normalizedOut = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      let value =
        terminalNormalizedSurface[field] !== undefined
    ...
    const rawSeed = {
      ...
      county_name:
        ...
        null,
      country_code:
        safeNullIfEmpty(terminalAddress.country_code) ||
        (hasMeaningfulAddressValue(terminalAddress.state_code) ? "US" : null),
      section: terminalAddress.section ?? section ?? null,
  ...
  // Ensure the final address aligns with a single oneOf branch: prefer normalized when
  // we have full coverage, otherwise emit the raw branch with a complete schema surface.
  lockFinalCountyAddressBranch(addressOutputPath);
  enforceFinalCountyAddressOneOf(addressOutputPath, {
    rawCandidates,
    requestIdentifier: resolvedRequestIdentifier,
    sourceHttpRequest: resolvedSourceHttp,
    fallbackCountyName:
  ...
  hardenFinalAddressOneOf(addressOutputPath, {
    rawCandidates,
    requestIdentifier: resolvedRequestIdentifier,
    sourceHttpRequest: resolvedSourceHttp,
    sourceHttpRequestCandidates: [
  ...
  // Ensure the terminal address payload cleanly selects oneOf: prefer normalized
  // only when fully populated, otherwise emit the raw branch with every schema
  // field present (nullable) anchored on the unnormalized string.
  if (fs.existsSync(addressOutputPath)) {
  ...
  finalizeAddressBranchSelection(addressOutputPath, {
  ...
  // Final, explicit oneOf selection: prefer the raw branch whenever we have an
  // unnormalized source string from the county; otherwise fall back to a fully
  // populated normalized payload.
  if (fs.existsSync(addressOutputPath)) {
  ...
  // Final guard: emit a single address payload that cleanly satisfies oneOf.
  const finalAddressSource = readJSONIfExists("unnormalized_address.json") || {};
  ...
  const finalRawValue = safeNullIfEmpty(
    resolveFirstNonEmptyString([
      ...
      fullAddr,
      fullAddrInput,
      finalAddressSource.full_address,
      finalAddressSource.unnormalized_address,
    ]),
  );
  ...
  if (finalNormalizedReady) {
    ...
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      ...
      if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
        ...
        value =
          safeNullIfEmpty(
            finalNormalizedSurface.request_identifier ??
              finalAddressSnapshot.request_identifier ??
              finalAddressSource.request_identifier ??
              (seed && seed.request_identifier),
          ) ?? null;
      } else if (field === "source_http_request") {
        value =
          resolveSourceHttpRequest(
            ...
            finalAddressSource.source_http_request,
            seed && seed.source_http_request,
          ) || null;
      } else if (typeof value === "string") {
        const trimmed = value.trim();
    ...
    const rawOut = { ...RAW_ADDRESS_SCHEMA_TEMPLATE };
    rawOut.unnormalized_address = finalRawValue;
    RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
      if (field === "unnormalized_address") return;
      let value =
        finalAddressSnapshot[field] !== undefined
  ...
  // Deterministically pick the final address oneOf branch: prefer the raw
  // variant when any unnormalized string exists, otherwise emit a fully
  // populated normalized payload. This keeps all schema fields present so the
  // relationship validator doesn't flag missing latitude/longitude or street
  // components.
  if (fs.existsSync(addressOutputPath)) {
    const terminalSnapshot = readJSONIfExists(addressOutputPath) || {};
  ...
  // Final oneOf clamp: prefer the raw branch when any unnormalized string exists
  // so validation doesn't expect normalized components; otherwise emit the fully
  // normalized surface. Drop everything if neither branch is satisfied.
  if (fs.existsSync(addressOutputPath)) {
    const terminalSnapshot = readJSONIfExists(addressOutputPath) || {};
    const normalizedSurface =
      ensureNormalizedAddressSchemaSurface &&
    ...
    if (terminalRaw) {
      ...
      RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
        ...
        if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
          const numeric = parseCoordinate(terminalSnapshot[field]);
          rawOut[field] = Number.isFinite(numeric) ? numeric : null;
          return;
        }
        if (field === "county_name") {
        ...
        if (field === "state_code") {
          const stateCandidate =
            safeNullIfEmpty(terminalSnapshot.state_code) ||
            safeNullIfEmpty(inferredStateCode) ||
            (seed && safeNullIfEmpty(seed.state_code)) ||
            "FL";
          rawOut[field] = stateCandidate;
          return;
        }
        rawOut[field] =
      ...
      NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
        ...
        if (field === "request_identifier") {
          value =
            safeNullIfEmpty(value ?? trimmedRequestIdentifier ?? parcelId) ??
            null;
        } else if (field === "source_http_request") {
          value =
            resolveSourceHttpRequest(
  ...
  // Final clamp: prefer the raw branch whenever any unnormalized string exists
  // and emit the full schema surface (nullable) so oneOf never demands missing
  // normalized fields. Only use normalized when it is fully populated.
  {
    const finalSnapshot = readJSONIfExists(addressOutputPath) || {};
    const rawBranchCandidate = safeNullIfEmpty(
      resolveFirstNonEmptyString([
    ...
    if (rawBranchCandidate) {
      const rawOut = { ...RAW_ADDRESS_SCHEMA_TEMPLATE };
      RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
        if (field === "unnormalized_address") {
          rawOut[field] = rawBranchCandidate;
          return;
        }
        if (field === "request_identifier") {
          rawOut[field] =
        ...
        if (field === "source_http_request") {
          rawOut[field] =
            resolveSourceHttpRequest(
              finalSnapshot.source_http_request,
              normalizedSurface && normalizedSurface.source_http_request,
              resolvedSourceHttp,
              sourceHttpCandidate,
              unnormalizedSource.source_http_request,
  ...
  enforceFinalCountyAddressOneOfSimple(addressOutputPath, {
    rawCandidates,
    requestIdentifier: resolvedRequestIdentifier,
    sourceHttpRequest: resolvedSourceHttp,
    countyFallback: formattedCountyName || countyName || "Palm Beach",
  ...
  // Final clamp: pick a single address branch with the full schema surface so
  // oneOf validation does not complain about missing normalized fields.
  {
    const terminalSnapshot = readJSONIfExists(addressOutputPath) || {};
    const terminalNormalizedSurface =
      ensureNormalizedAddressSchemaSurface &&
    ...
    if (terminalNormalizedReady) {
      ...
      const preparedSource = prepareSourceHttpRequest(
        resolveSourceHttpRequest(
          terminalSnapshot.source_http_request,
          terminalNormalizedSurface && terminalNormalizedSurface.source_http_request,
          resolvedSourceHttp,
          sourceHttpCandidate,
        ),
      );
      ...
      if (baseRaw) {
        if (!baseRaw.postal_code) {
          baseRaw.plus_four_postal_code = null;
        }
        if ((baseRaw.latitude == null) !== (baseRaw.longitude == null)) {
          baseRaw.latitude = null;
          baseRaw.longitude = null;
        }
        if (
          hasMeaningfulAddressValue(baseRaw.state_code) &&
          !hasMeaningfulAddressValue(baseRaw.country_code)
        ) {
          baseRaw.country_code = "US";
        }
        writeJSON(addressOutputPath, baseRaw);
      } else {
        removeFileIfExists(addressOutputPath);
      }
    } else {
  ...
  finalizeAddressBranchForOutput(addressOutputPath, {
    rawCandidates,
    requestIdentifier: resolvedRequestIdentifier,
    sourceHttpRequest: resolvedSourceHttp || sourceHttpCandidate,
    countyFallback: formattedCountyName || countyName || "Palm Beach",
  ...
  // Finalize the address onto a single branch: prefer raw when only an
  // unnormalized string is available so the validator sees every required
  // field (nullable) rather than a partial normalized payload.
  {
    const snapshot = readJSONIfExists(addressOutputPath) || {};
    const normalizedSurface =
      ensureNormalizedAddressSchemaSurface &&
    ...
    if (!normalizedComplete && rawCandidate) {
      const rawOut =
        ensureRawAddressSchemaDefaults(
          {
            ...snapshot,
          ...
          RAW_ADDRESS_ALLOWED_FIELDS,
        ) || { ...RAW_ADDRESS_SCHEMA_TEMPLATE, unnormalized_address: rawCandidate };
  ...
  commitCanonicalAddressOneOf(addressOutputPath, {
    rawCandidates: [
      ...rawCandidates,
      canonicalUnnormalized,
      unAddr && unAddr.full_address,
  ...
  // Final clamp: prefer the raw/unnormalized branch whenever the source provides it;
  // otherwise emit the normalized branch only when it is complete. This keeps the
  // address on a single oneOf branch per the schema.
  {
    const snapshot = readJSONIfExists(addressOutputPath) || {};
    const normalizedSurface =
      ensureNormalizedAddressSchemaSurface &&
        ...
        if (field === "request_identifier") {
          const candidate =
            resolvedRequestIdentifier === undefined
              ? safeNullIfEmpty(snapshot.request_identifier)
              : resolvedRequestIdentifier;
          rawOut[field] = candidate === undefined ? null : candidate;
          return;
        }
        if (field === "source_http_request") {
          rawOut[field] =
  ...
  // Final deterministic oneOf selection: prefer the raw branch whenever an
  // unnormalized address string is present, and emit every schema field
  // (nullable) so validation never complains about missing latitude/street keys.
  if (fs.existsSync(addressOutputPath)) {
    const snapshot = readJSONIfExists(addressOutputPath) || {};
    const normalizedSurface =
      ensureNormalizedAddressSchemaSurface &&
    ...
    if (rawCandidate) {
      ...
      RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
        if (field === "unnormalized_address") {
          finalRawOut[field] = rawCandidate;
          return;
        }
        if (field === "request_identifier") {
          finalRawOut[field] =
        ...
        if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
          const numeric = parseCoordinate(snapshot[field]);
          finalRawOut[field] = Number.isFinite(numeric) ? numeric : null;
          return;
        }
        // Leave all other fields as null to stay on the raw oneOf surface.
      });
      if (!finalRawOut.postal_code) {
        finalRawOut.plus_four_postal_code = null;
      }
      ...
      writeJSON(addressOutputPath, finalRawOut);
    } else if (normalizedReady) {
      const normalizedOut = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
      NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
        let value =
  ...
  enforceSourcePreferredAddressBranch(addressOutputPath, {
    rawCandidates,
    requestIdentifier:
      resolvedRequestIdentifier ??
      trimmedRequestIdentifier ??
  ...
  enforceAddressTemplateCoverage(addressOutputPath, {
    defaultCounty: formattedCountyName || countyName || "Palm Beach",
    defaultState: inferredStateCode || "FL",
    defaultCountry: "US",
  });
  coerceAddressToSingleBranch(addressOutputPath, { defaultCountry: "US" });
  ...
  // Final guard: if we don't have a complete normalized address, emit a raw
  // branch payload that cleanly satisfies the oneOf by anchoring on
  // unnormalized_address while keeping the full schema surface nullable.
  const existingFinalAddress = readJSONIfExists(addressOutputPath) || {};
  const normalizedSurfaceCandidate =
    ensureNormalizedAddressSchemaSurface &&
    ensureNormalizedAddressSchemaSurface({
      ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE,
      ...existingFinalAddress,
    });
  const hasNormalizedBranch =
    normalizedSurfaceCandidate &&
    hasCompleteNormalizedAddress({ ...normalizedSurfaceCandidate });

  const finalRawSnapshot = existingFinalAddress || {};
  const finalRawString =
    safeNullIfEmpty(finalRawSnapshot.unnormalized_address) ||
    safeNullIfEmpty(unAddr && unAddr.full_address) ||
    safeNullIfEmpty(unAddr && unAddr.unnormalized_address) ||
    null;
  const finalRequestId =
    safeNullIfEmpty(
      finalRawSnapshot.request_identifier ??
        (typeof resolvedRequestIdentifier !== "undefined"
          ? resolvedRequestIdentifier
          : null) ??
        trimmedRequestIdentifier ??
        parcelId ??
  ...
  const finalSourceHttp = resolveSourceHttpRequest(
    finalRawSnapshot.source_http_request,
    typeof resolvedSourceHttp !== "undefined" ? resolvedSourceHttp : null,
    seed && seed.source_http_request,
    unAddr && unAddr.source_http_request,
  );
  const finalLat = parseCoordinate(
    finalRawSnapshot.latitude ?? unAddr.latitude ?? null,
  );
  const finalLon = parseCoordinate(
    finalRawSnapshot.longitude ?? unAddr.longitude ?? null,
  );

  if (!hasNormalizedBranch && finalRawString) {
    const rawOut = { ...RAW_ADDRESS_SCHEMA_TEMPLATE };

    RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
      if (field === "unnormalized_address") return;
      if (!Object.prototype.hasOwnProperty.call(finalRawSnapshot, field)) {
        return;
      }
      let value = finalRawSnapshot[field];
      if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
        const numeric = parseCoordinate(value);
        value = Number.isFinite(numeric) ? numeric : null;
      } else if (typeof value === "string") {
    ...
    rawOut.unnormalized_address = finalRawString.trim();
    rawOut.request_identifier = finalRequestId;
    rawOut.source_http_request = prepareSourceHttpRequest(finalSourceHttp) || null;
    rawOut.county_name =
      rawOut.county_name ||
      formattedCountyName ||
      countyName ||
      "Palm Beach";
    rawOut.state_code = rawOut.state_code || inferredStateCode || "FL";
    rawOut.country_code =
      rawOut.country_code ||
      ((rawOut.state_code || inferredStateCode) ? "US" : null);
    rawOut.latitude = Number.isFinite(finalLat) ? finalLat : null;
    rawOut.longitude = Number.isFinite(finalLon) ? finalLon : null;

    rawOut.postal_code =
      rawOut.postal_code ?? safeNullIfEmpty(finalRawSnapshot.postal_code);
    rawOut.plus_four_postal_code = rawOut.postal_code
      ? rawOut.plus_four_postal_code ??
        safeNullIfEmpty(finalRawSnapshot.plus_four_postal_code) ??
        null
      : null;
    ...
    writeJSON(addressOutputPath, rawOut);
  } else if (hasNormalizedBranch) {
    const normalizedOut = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      if (field === "request_identifier") {
        normalizedOut[field] = finalRequestId === undefined ? null : finalRequestId;
        return;
      }
      if (field === "source_http_request") {
        normalizedOut[field] = prepareSourceHttpRequest(finalSourceHttp) || null;
        return;
      }
      let value = normalizedSurfaceCandidate[field];
      if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
        const numeric = parseCoordinate(
          value !== undefined ? value : field === "latitude" ? finalLat : finalLon,
        );
        normalizedOut[field] = Number.isFinite(numeric) ? numeric : null;
        return;
      }
  ...
  // Re-clamp the final address payload to a single oneOf branch. Prefer the raw
  // branch when the source supplied an unnormalized string; otherwise emit the
  // normalized surface without the raw field. Keep every schema field present
  // (nullable) so validators don't complain about missing latitude or street keys.
  const finalOneOfSnapshot = readJSONIfExists(addressOutputPath) || {};
  const finalOneOfRaw = safeNullIfEmpty(
    resolveFirstNonEmptyString([
  ...
  const finalOneOfSourceHttp = resolveSourceHttpRequest(
  ...
  // Final cleanup: drop any locally generated address relationship files and
  // keep property relationship placeholders null so downstream can populate URIs.
  purgeAddressRelationshipArtifacts(dataDir);
  purgeAddressRelationshipArtifacts(relationshipsDir);
  removeAddressRelationshipFiles(dataDir);
  removeAddressRelationshipFiles(relationshipsDir);
  ...
  // Deterministically lock the terminal address payload to a single oneOf branch.
  // Prefer the raw branch whenever we have an unnormalized source string; otherwise
  // emit the normalized payload with the raw field removed.
  if (fs.existsSync(addressOutputPath)) {
    const terminalSnapshot = readJSONIfExists(addressOutputPath) || {};
    const resolvedRawAddress = safeNullIfEmpty(
      resolveFirstNonEmptyString([
    ...
    if (resolvedRawAddress) {
      rebuilt = { ...RAW_ADDRESS_SCHEMA_TEMPLATE };
      RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
        if (field === "unnormalized_address") {
          rebuilt[field] = resolvedRawAddress;
          return;
        }
        if (field === "request_identifier") {
          rebuilt[field] = resolvedRequestId === undefined ? null : resolvedRequestId;
          return;
        }
        if (field === "source_http_request") {
          const prepared = prepareSourceHttpRequest(resolvedSourceHttpRequest);
        ...
        if (field === "county_name") {
          rebuilt[field] = fallbackCounty || null;
          return;
        }
        if (field === "state_code") {
          rebuilt[field] = fallbackState || null;
          return;
        }
        ...
        if (typeof value === "string") {
          const trimmed = value.trim();
          const sanitized =
            trimmed.length && sanitizeAddressFieldValue
              ? sanitizeAddressFieldValue(field, trimmed)
              : trimmed.length
                ? trimmed
                : null;
          rebuilt[field] = sanitized;
          return;
        }
      ...
      rebuilt = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
      NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
        if (field === "request_identifier") {
          rebuilt[field] = resolvedRequestId === undefined ? null : resolvedRequestId;
          return;
      ...
      if (
        Object.prototype.hasOwnProperty.call(rebuilt, "unnormalized_address")
      ) {
    ...
    if (rebuilt) {
      writeJSON(addressOutputPath, rebuilt);
    } else {
      removeFileIfExists(addressOutputPath);
    }
  }

  // Deterministically clamp the terminal address payload to a single oneOf branch.
  // Prefer the raw (unnormalized) branch whenever a source string exists so we
  // keep every schema field present (nullable) and avoid normalized-required
  // validation errors.
  const finalTerminalSnapshot = readJSONIfExists(addressOutputPath) || {};
  const finalTerminalNormalizedSurface =
    ensureNormalizedAddressSchemaSurface &&
    ensureNormalizedAddressSchemaSurface({ ...finalTerminalSnapshot });
  const finalTerminalNormalizedComplete =
    finalTerminalNormalizedSurface &&
    hasCompleteNormalizedAddress({ ...finalTerminalNormalizedSurface });
  const finalTerminalRawCandidate = safeNullIfEmpty(
    resolveFirstNonEmptyString([
      finalTerminalSnapshot.unnormalized_address,
      ...rawCandidates,
      canonicalUnnormalized,
      unAddr && unAddr.full_address,
      unAddr && unAddr.unnormalized_address,
  ...
  if (finalTerminalRawCandidate) {
    ...
    RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
      if (field === "unnormalized_address") {
        finalRawOut[field] = finalTerminalRawCandidate;
        return;
      }
      if (field === "request_identifier") {
        finalRawOut[field] =
          safeNullIfEmpty(
            resolveFirstNonEmptyString([
              finalTerminalSnapshot.request_identifier,
              resolvedRequestIdentifier,
              trimmedRequestIdentifier,
              parcelId,
              seed && seed.request_identifier,
      ...
      if (field === "source_http_request") {
        finalRawOut[field] =
          prepareSourceHttpRequest(
            resolveSourceHttpRequest(
              finalTerminalSnapshot.source_http_request,
              finalTerminalNormalizedSurface &&
                finalTerminalNormalizedSurface.source_http_request,
              unAddr && unAddr.source_http_request,
              seed && seed.source_http_request,
            ),
          ) || null;
      ...
      let value =
        finalTerminalSnapshot[field] !== undefined
          ? finalTerminalSnapshot[field]
          : finalTerminalNormalizedSurface
            ? finalTerminalNormalizedSurface[field]
            : null;
      if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
        const numeric = parseCoordinate(value);
        finalRawOut[field] = Number.isFinite(numeric) ? numeric : null;
      ...
      if (typeof value === "string") {
        const trimmed = value.trim();
        finalRawOut[field] = trimmed.length ? trimmed : null;
        return;
      }
      finalRawOut[field] = value === undefined ? null : value;
    });

    if (!finalRawOut.postal_code) {
      finalRawOut.plus_four_postal_code = null;
    }
    if ((finalRawOut.latitude == null) !== (finalRawOut.longitude == null)) {
      finalRawOut.latitude = null;
      finalRawOut.longitude = null;
    }
    if (
    ...
    writeJSON(addressOutputPath, finalRawOut);
  } else if (finalTerminalNormalizedComplete) {
    const normalizedOut = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      let value =
        finalTerminalNormalizedSurface[field] !== undefined
          ? finalTerminalNormalizedSurface[field]
          : finalTerminalSnapshot[field];
      if (field === "request_identifier") {
        value =
  ...
  // Final deterministic oneOf resolution: prefer normalized only when the
  // schema-required normalized surface is present; otherwise emit a lean raw
  // branch anchored on the unnormalized address string. Keep only schema-legal
  // fields to avoid triggering normalized oneOf validation when raw is used.
  if (fs.existsSync(addressOutputPath)) {
    const finalSnapshot = readJSONIfExists(addressOutputPath) || {};
    const finalNormalizedSurface =
      ensureNormalizedAddressSchemaSurface &&
    ...
    const finalNormalizedReady =
      finalNormalizedSurface &&
      hasCompleteNormalizedAddress({ ...finalNormalizedSurface });
    finalRawCandidate = safeNullIfEmpty(
      resolveFirstNonEmptyString([
        finalSnapshot.unnormalized_address,
        ...rawCandidates,
        unnormalizedSource.unnormalized_address,
    ...
    const finalSourceHttp =
      prepareSourceHttpRequest(
        resolveSourceHttpRequest(
          finalSnapshot.source_http_request,
          finalNormalizedSurface && finalNormalizedSurface.source_http_request,
          resolvedSourceHttp,
          sourceHttpCandidate,
          unnormalizedSource.source_http_request,
          seedSource && seedSource.source_http_request,
    ...
    if (finalNormalizedReady) {
      ...
      NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
        ...
        if (field === "request_identifier") {
          value =
            finalRequestId === undefined ? safeNullIfEmpty(value) : finalRequestId;
        } else if (field === "source_http_request") {
          value = finalSourceHttp;
        } else if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
          const numeric = parseCoordinate(value);
          value = Number.isFinite(numeric) ? numeric : null;
        } else if (typeof value === "string") {
      ...
      if (finalSourceHttp) {
        rawOut.source_http_request = finalSourceHttp;
      }

      const resolvedCounty = safeNullIfEmpty(
        resolveFirstNonEmptyString([
          finalSnapshot.county_name,
          inferredCounty,
          formattedCountyName,
          countyName,
        ]),
      ...
      if (resolvedCounty) {
        rawOut.county_name = resolvedCounty;
      }

      const resolvedState = safeNullIfEmpty(
        resolveFirstNonEmptyString([
          finalSnapshot.state_code,
          inferredStateCode,
          "FL",
        ]),
      );
      if (resolvedState) {
        rawOut.state_code = resolvedState;
      }
      const resolvedCountry =
        hasMeaningfulAddressValue(resolvedState) ||
        hasMeaningfulAddressValue(finalSnapshot.country_code)
          ? safeNullIfEmpty(finalSnapshot.country_code) || "US"
          : safeNullIfEmpty(finalSnapshot.country_code);
      if (resolvedCountry) {
  ...
  clampCountyAddressOneOf(addressOutputPath, {
    requestIdentifier:
      resolvedRequestIdentifier ?? trimmedRequestIdentifier ?? parcelId ?? null,
    sourceHttpRequest: resolvedSourceHttp ?? sourceHttpCandidate ?? null,
  });

  finalizeCountyAddressPayload(addressOutputPath, {
    rawCandidates: [
      finalRawCandidate,
    ...
    sourceHttpRequest: resolvedSourceHttp ?? sourceHttpCandidate ?? null,
    defaults: {
      county_name: inferredCounty || formattedCountyName || countyName || null,
      state_code: inferredStateCode || "FL",
      country_code: "US",
    },
  });
  enforceRawPreferredAddressBranch(addressOutputPath, {
    defaultCounty: inferredCounty || formattedCountyName || countyName || null,
    defaultState: inferredStateCode || "FL",
    defaultCountry: "US",
  });

  // One last clamp: pick a single oneOf branch with the full schema surface.
  // Prefer normalized only when it is complete; otherwise emit the raw branch
  // anchored on the unnormalized string so required fields stay present.
  if (fs.existsSync(addressOutputPath)) {
    const snapshot = readJSONIfExists(addressOutputPath) || {};
    const normalizedSurface =
      typeof ensureNormalizedAddressSchemaSurface === "function"
    ...
    if (normalizedReady && !rawCandidate) {
      const normalizedOut = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
      NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
        let value =
          snapshot[field] !== undefined ? snapshot[field] : normalizedSurface[field];
  ...
  finalizeAddressBranchSelection(addressOutputPath, {
    ...
    requestIdentifier:
      resolvedRequestIdentifier ?? trimmedRequestIdentifier ?? parcelId ?? null,
    sourceHttpRequest: resolvedSourceHttp ?? sourceHttpCandidate ?? null,
    fallbackCounty: formattedCountyName || countyName || "Palm Beach",
    fallbackState: inferredStateCode || "FL",
    fallbackCountry: "US",
  });

  // Safety clamp: prefer the raw oneOf branch when any unnormalized address
  // string exists, and always emit the full schema surface so required fields
  // are present (nullable) instead of triggering normalized validation errors.
  if (fs.existsSync(addressOutputPath)) {
    const coverageSnapshot = readJSONIfExists(addressOutputPath) || {};
    const rawPriority = safeNullIfEmpty(
      resolveFirstNonEmptyString([
        coverageSnapshot.unnormalized_address,
        ...(rawCandidates || []),
        canonicalUnnormalized,
        finalRawCandidate,
        finalRawCandidateClamp,
    ...
    const coverageNormalized =
      ensureNormalizedAddressSchemaSurface &&
      ensureNormalizedAddressSchemaSurface({ ...coverageSnapshot });
    const coverageNormalizedReady =
      coverageNormalized &&
      hasCompleteNormalizedAddress({ ...coverageNormalized });
      ...
      RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
        ...
        if (field === "request_identifier") {
          finalOut[field] =
            safeNullIfEmpty(
              resolveFirstNonEmptyString([
                coverageSnapshot.request_identifier,
                resolvedRequestIdentifier,
                trimmedRequestIdentifier,
                parcelId,
                seed && seed.request_identifier,
        ...
        if (field === "source_http_request") {
          finalOut[field] =
            prepareSourceHttpRequest(
              resolveSourceHttpRequest(
                coverageSnapshot.source_http_request,
                coverageNormalized && coverageNormalized.source_http_request,
                resolvedSourceHttp,
                sourceHttpCandidate,
                unAddr && unAddr.source_http_request,
                seed && seed.source_http_request,
        ...
        let value =
          coverageSnapshot[field] !== undefined
            ? coverageSnapshot[field]
            : coverageNormalized && coverageNormalized[field];
    ...
    } else if (coverageNormalizedReady) {
      finalOut = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
      NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
        let value =
          coverageNormalized && coverageNormalized[field] !== undefined
            ? coverageNormalized[field]
            : coverageSnapshot[field];
        ...
        if (field === "request_identifier") {
          ...
          value =
            prepareSourceHttpRequest(
              resolveSourceHttpRequest(
                coverageSnapshot.source_http_request,
                coverageNormalized.source_http_request,
                resolvedSourceHttp,
                sourceHttpCandidate,
                unAddr && unAddr.source_http_request,
                seed && seed.source_http_request,
    ...
    if (finalOut) {
      writeJSON(addressOutputPath, finalOut);
    } else {
      removeFileIfExists(addressOutputPath);
    }
  }

  // Final clean-up: keep property address relationships null and drop any
  // locally generated address relationship artifacts (URs are populated
  // downstream).
  stabilizeFinalAddressBranch(addressOutputPath, {
    county: formattedCountyName || countyName || "Palm Beach",
    state: inferredStateCode || "FL",
    country: "US",
  });
  lockRawAddressOneOf(addressOutputPath, {
    requestIdentifier:
      resolvedRequestIdentifier ?? trimmedRequestIdentifier ?? parcelId ?? null,
    sourceHttpRequest: resolvedSourceHttp ?? sourceHttpCandidate ?? null,
    county: formattedCountyName || countyName || "Palm Beach",
  ...
  finalizeCountyOneOfBranch(addressOutputPath, {
    rawFallbacks: [
      finalRawCandidate,
      finalRawCandidateClamp,
      finalRawString,

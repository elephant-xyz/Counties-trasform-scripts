   ...
   try {
     ...
     if (normalizedTarget.endsWith(`${path.sep}address.json`) || normalizedTarget === "address.json") {
       ...
       if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
         ...
         } else if (parsed.unnormalized_address) {
           finalPayload =
...
async function main() {
  ...
  try {
    ...
    console.error("Failed to collapse address payload to minimal raw surface:", error);
    if (!process.exitCode) {
      process.exitCode = 1;
    }
  }
  ...
  try {
    const stabilized = enforceFinalRawAddressSkeleton(addressOutputPath, {
      unnormalizedPath: "unnormalized_address.json",
      seedPath: "property_seed.json",
    ...
    console.error("Failed to enforce terminal raw address skeleton:", error);
    if (!process.exitCode) {
      process.exitCode = 1;
    }
  }
  ...
  try {
    enforceTerminalAddressOneOfSelection(addressOutputPath, {
    ...
    console.error("Failed to emit final address oneOf-compliant payload:", error);
    if (!process.exitCode) {
      process.exitCode = 1;
    }
  }
  ...
  try {
    enforceMinimalRawAddressSurface(addressOutputPath);
    enforcePropertyRelationshipNulls(propertyFilePath);
    relationshipDirs.forEach((dirPath) => {
      ensureNullRelationshipPlaceholders(dirPath, managedBaseNames);
    });
  ...
  try {
    ...
    console.error("Failed to finalize schema-compliant address payload:", error);
    if (!process.exitCode) {
      process.exitCode = 1;
    }
  }
  ...
  try {
    ...
    console.error("Failed to finalize address branch for oneOf:", error);
    if (!process.exitCode) {
      process.exitCode = 1;
    }
  }
  ...
  try {
    forceFinalAddressOneOfCompliance(
      addressOutputPath,
      relationshipDirs,
      propertyFilePath,
    );
  } catch (error) {
    console.error("Failed to force terminal address oneOf compliance:", error);
    if (!process.exitCode) {
      process.exitCode = 1;
    }
  }
  ...
  try {
    ...
    console.error("Failed to enforce terminal address branch selection:", error);
    if (!process.exitCode) {
      process.exitCode = 1;
    }
  }
  ...
  try {
    stabilizeAddressOneOfForOutput(addressOutputPath);
    enforcePropertyRelationshipNulls(propertyFilePath);
    relationshipDirs.forEach((dirPath) => {
      ensureNullRelationshipPlaceholders(dirPath, managedBaseNames);
    });
  ...
  try {
    enforceAddressBranchForOneOf(addressOutputPath);
    enforcePropertyRelationshipNulls(propertyFilePath);
    [dataDir, relationshipsDir].forEach((dirPath) => {
      ensureNullRelationshipPlaceholders(dirPath, managedBaseNames);
    });
  ...
  try {
    ...
    if (hasNormalized) {
      ...
      if (Object.prototype.hasOwnProperty.call(addressPayload, "request_identifier")) {
        const requestIdentifier = safeNullIfEmpty(addressPayload.request_identifier);
        normalizedOut.request_identifier =
          requestIdentifier === undefined ? null : requestIdentifier;
      }

      if (Object.prototype.hasOwnProperty.call(addressPayload, "source_http_request")) {
        const preparedSource = prepareSourceHttpRequest(addressPayload.source_http_request);
        normalizedOut.source_http_request = preparedSource
          ? deepClone(preparedSource)
          : null;
      }
      ...
      if (rawAddress) {
        ...
        for (const field of RAW_ADDRESS_ALLOWED_FIELDS) {
          ...
          let candidate = Object.prototype.hasOwnProperty.call(addressPayload, field)
            ? addressPayload[field]
            : null;

          if (field === "county_name" && !candidate) {
  ...
  try {
    enforceAddressSchemaBranchForOutput(addressOutputPath, {
      rawCandidates: [
        ...finalUnnormalizedCandidates,
        unnormalizedAddressCandidate,
        combinedModelAddress,
    ...
    enforceRawAddressOneOfSelection(addressOutputPath, {
      unnormalizedCandidates: [
        ...finalUnnormalizedCandidates,
        unnormalizedAddressCandidate,
        combinedModelAddress,
    ...
    console.error("Failed to enforce terminal address schema branch for output:", error);
    if (!process.exitCode) {
      process.exitCode = 1;
    }
  }
  ...
  stabilizeCountyAddressOneOf(addressOutputPath, {
    ...
    relationshipDirs,
    defaultCountyName: formattedCountyName || countyName || defaultCounty,
    defaultCountryCode: "US",
    rawCandidates: [
      ...finalUnnormalizedCandidates,
      unnormalizedAddressCandidate,
  ...
  forceMinimalRawAddress(addressOutputPath, {
    unnormalizedCandidates: [
      ...finalUnnormalizedCandidates,
      unnormalizedAddressCandidate,
      combinedModelAddress,
    ...
    sourceHttpRequest: sourceHttpCandidate || null,
    sourceHttpRequestCandidates: [
      seed && seed.source_http_request,
      unAddr && unAddr.source_http_request,
    ],
    countyName: formattedCountyName || countyName || defaultCounty,
    defaultCountryCode: "US",
    coordinateCandidates: [
      { latitude: initialLatitude, longitude: initialLongitude },
      parcelCentroid,
  ...
  try {
    enforceRawAddressSurface(addressOutputPath);
    ensureRawAddressFieldPresence(addressOutputPath);
  } catch (error) {
    console.error("Failed to enforce raw address surface:", error);
    if (!process.exitCode) {
      process.exitCode = 1;
    }
  }
  ...
  try {
    const rawAddressValue = resolveFirstNonEmptyString([
      unnormalizedAddressCandidate,
      ...finalUnnormalizedCandidates,
      addressLineCombined,
      siteLocationLine,
    ...
    const resolvedCoordinates =
      Number.isFinite(initialLatitude) && Number.isFinite(initialLongitude)
        ? { latitude: initialLatitude, longitude: initialLongitude }
        : parcelCentroid;
    const resolvedCountyName = formattedCountyName || countyName || defaultCounty;
    ...
    writeRawAddressForSchema({
      addressPath: addressOutputPath,
    ...
    enforceFinalCountyAddressVariant(addressOutputPath);
  ...
  try {
    emitFinalCountyAddressOneOf(addressOutputPath, {
      rawCandidates: [
        ...finalUnnormalizedCandidates,
        unnormalizedAddressCandidate,
        combinedModelAddress,
      ...
      fieldFallbacks: {
        county_name: [formattedCountyName, countyName, "Palm Beach"],
        state_code: [countyInferredStateCode, resolvedStateUpper, "FL"],
        country_code: ["US"],
        city_name: [normalizedCity, resolvedCity],
        municipality_name: [
          normalizedMunicipality ? toTitleCase(normalizedMunicipality) : null,
          resolvedCity,
        ],
        postal_code: [sanitizedPostalCode, fallbackPostalValue, postalCode],
        plus_four_postal_code: [sanitizedPlus4, fallbackPlus4Value, plus4],
      },
      defaultCountryCode: "US",
    });
    enforcePropertyRelationshipNulls(propertyFilePath);
    ...
    console.error("Failed to emit final county address oneOf payload:", error);
    if (!process.exitCode) {
      process.exitCode = 1;
    }
  }
  ...
  try {
    ...
    const rawSnapshot = buildRawAddressSnapshot({
      rawCandidates: [
        ...finalUnnormalizedCandidates,
        unnormalizedAddressCandidate,
        combinedModelAddress,
      ...
      stateFallback: resolvedStateUpper || countyInferredStateCode || "FL",
      countryFallback: "US",
    });

    lockedAddressPath = null;
    if (rawSnapshot) {
      const enrichedSnapshot = { ...rawSnapshot };
      const rawAddressString = safeNullIfEmpty(
        resolveFirstNonEmptyString([
          rawSnapshot.unnormalized_address,
          addressLineCombined,
          siteLocationLine,
          combinedModelAddress,
          fullAddr,
      ...
      if (rawAddressString) {
        const parsedStreet = parseLocationAddress(
          rawAddressString.split(",")[0] || rawAddressString,
        );
        const parsedCityState = parseCityStatePostal(rawAddressString);
        if (parsedStreet) {
          if (!hasMeaningfulAddressValue(enrichedSnapshot.street_number)) {
            enrichedSnapshot.street_number = parsedStreet.streetNumber || null;
          }
          if (!hasMeaningfulAddressValue(enrichedSnapshot.street_name)) {
            enrichedSnapshot.street_name = parsedStreet.streetName || null;
          }
          if (
            !hasMeaningfulAddressValue(enrichedSnapshot.street_pre_directional_text)
          ) {
            enrichedSnapshot.street_pre_directional_text =
              parsedStreet.streetPreDirectional || null;
          }
          if (
            !hasMeaningfulAddressValue(enrichedSnapshot.street_post_directional_text)
          ) {
            enrichedSnapshot.street_post_directional_text =
              parsedStreet.streetPostDirectional || null;
          }
          if (!hasMeaningfulAddressValue(enrichedSnapshot.street_suffix_type)) {
            enrichedSnapshot.street_suffix_type = parsedStreet.streetSuffix || null;
          }
          if (!hasMeaningfulAddressValue(enrichedSnapshot.unit_identifier)) {
            enrichedSnapshot.unit_identifier = parsedStreet.unitIdentifier || null;
          }
          if (!hasMeaningfulAddressValue(enrichedSnapshot.route_number)) {
            enrichedSnapshot.route_number = parsedStreet.routeNumber || null;
          }
        }
        if (parsedCityState) {
          if (!hasMeaningfulAddressValue(enrichedSnapshot.city_name)) {
            enrichedSnapshot.city_name = parsedCityState.city || null;
          }
          if (!hasMeaningfulAddressValue(enrichedSnapshot.state_code)) {
            enrichedSnapshot.state_code = parsedCityState.state || null;
          }
          if (!hasMeaningfulAddressValue(enrichedSnapshot.postal_code)) {
            enrichedSnapshot.postal_code = parsedCityState.postal || null;
          }
          if (!hasMeaningfulAddressValue(enrichedSnapshot.plus_four_postal_code)) {
            enrichedSnapshot.plus_four_postal_code = parsedCityState.plus4 || null;
          }
        }
      }
      ...
      const normalizedSurface =
        ensureNormalizedAddressSchemaSurface &&
        ensureNormalizedAddressSchemaSurface({ ...enrichedSnapshot });
      const normalizedReady =
        normalizedSurface &&
        NORMALIZED_ADDRESS_REQUIRED_STRING_FIELDS.every((field) =>
          hasMeaningfulAddressValue(normalizedSurface[field]),
      ...
      if (normalizedReady) {
        ...
        normalizedOut.request_identifier = enrichedSnapshot.request_identifier ?? null;
        normalizedOut.source_http_request = enrichedSnapshot.source_http_request
          ? deepClone(enrichedSnapshot.source_http_request)
          : null;
        if (
        ...
        finalAddressPayload = normalizedOut;
      } else {
        const rawOut =
          ensureRawAddressSchemaDefaults({ ...enrichedSnapshot }) || {
            ...RAW_ADDRESS_SCHEMA_TEMPLATE,
            ...enrichedSnapshot,
          };
        RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
      ...
      if (normalizedReady) {
        const verifyNormalized = readJSONIfExists(addressOutputPath) || finalAddressPayload || {};
        if (verifyNormalized && typeof verifyNormalized === "object") {
          if (Object.prototype.hasOwnProperty.call(verifyNormalized, "unnormalized_address")) {
            delete verifyNormalized.unnormalized_address;
          }
          NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
            if (!Object.prototype.hasOwnProperty.call(verifyNormalized, field)) {
              verifyNormalized[field] = null;
            }
          });
          if (!verifyNormalized.postal_code) {
            verifyNormalized.plus_four_postal_code = null;
          }
          if (
            hasMeaningfulAddressValue(verifyNormalized.state_code) &&
            !hasMeaningfulAddressValue(verifyNormalized.country_code)
          ) {
            verifyNormalized.country_code = "US";
          }
          if (
            (verifyNormalized.latitude == null) !==
            (verifyNormalized.longitude == null)
          ) {
            verifyNormalized.latitude = null;
            verifyNormalized.longitude = null;
          }
          fs.writeFileSync(
            addressOutputPath,
            `${JSON.stringify(verifyNormalized, null, 2)}\n`,
          );
          lockedAddressPath = path.resolve(addressOutputPath);
        }
      }
    ...
    console.error("Failed to rewrite unnormalized address payload:", error);
    if (!process.exitCode) {
      process.exitCode = 1;
    }
  }
  ...
  try {
    lockedAddressPath = null;
    enforceRawMinimalAddressOutput(addressOutputPath, {
      unnormalizedPath: "unnormalized_address.json",
      seedPath: "property_seed.json",
  ...
  // Final pass: pick a single address branch (normalized if fully present, otherwise raw-only)
  // and drop any relationship artifacts that should be auto-generated downstream.
  try {
    const existingAddress = readJSONIfExists(addressOutputPath) || {};
    const normalizedSurface =
      ensureNormalizedAddressSchemaSurface &&
  ...
  // Explicit oneOf selection: choose normalized when complete, otherwise emit a raw branch
  // with unnormalized_address and schema-aligned nullable fields.
  try {
    const addressPath = path.join(dataDir, "address.json");
    const relationshipsDir = path.join("relationships");
      ...
      if (normalizedReady) {
        ...
        NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
          ...
          if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
            ...
            normalizedOut[field] = candidate === undefined ? null : candidate;
          }
        });
        ...
        const rawSeed = {
          ...
          request_identifier:
            resolvedRequestId === undefined ? null : resolvedRequestId,
          source_http_request: payload.source_http_request || null,
        };
        const rawOut =
          enforceRawVariantAllowedFields(rawSeed) ||
          ensureRawAddressSchemaDefaults({
            ...RAW_ADDRESS_SCHEMA_TEMPLATE,
            ...rawSeed,
          }) || { ...RAW_ADDRESS_SCHEMA_TEMPLATE, ...rawSeed };
  ...
  // Final pass: collapse the address into the raw oneOf branch using the source
  // unnormalized string and strip normalized fields to avoid mixed-branch payloads.
  try {
    const addressPath = path.join(dataDir, "address.json");
    if (fs.existsSync(addressPath)) {
      const existingAddress = readJSONIfExists(addressPath) || {};
      const rawString = safeNullIfEmpty(
        resolveFirstNonEmptyString([
          existingAddress.unnormalized_address,
          unnormalizedAddressCandidate,
          fullAddrInput,
      ...
      if (rawString) {
        ...
        if (countyCandidate) {
          minimalRaw.county_name = titleCaseCounty(countyCandidate);
  ...
  // Final guard: prefer the raw address branch using the source-provided unnormalized string
  // and strip any auto-generated address relationships so the pipeline can populate them.
  try {
    const addressPath = path.join(dataDir, "address.json");
    const finalRawPayload = buildRawAddressFromSource({
      addressPath,
      unnormalizedSource: unAddr,
      seed,
      parcelId,
  ...
  try {
    hydrateRawAddressSkeleton(addressOutputPath, {
      defaultCountyName: formattedCountyName || countyName || "Palm Beach",
    });
    enforcePropertyRelationshipNulls(propertyFilePath);
    [dataDir, relationshipsDir].forEach((dirPath) => {
    ...
    console.error("Failed to hydrate final raw address skeleton:", error);
    if (!process.exitCode) {
      process.exitCode = 1;
    }
  }

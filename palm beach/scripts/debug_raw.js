...
async function main() {
...

  {
    ...
    // Final single-branch enforcement: prefer the raw branch whenever an
    // unnormalized string exists; only emit the normalized branch when fully
    // populated and no raw value is available.
    const finalBranchSnapshot = readJSONIfExists(addressOutputPath) || {};
    const finalBranchRaw = safeNullIfEmpty(
      resolveFirstNonEmptyString([
        finalBranchSnapshot.unnormalized_address,
        finalRawCandidateClamp,
        strictRawCandidate,
        finalClampRawValue,
        ...(finalUnnormalizedCandidates || []),
    ...
    const finalBranchNormalizedSurface =
      ensureNormalizedAddressSchemaSurface &&
      ensureNormalizedAddressSchemaSurface({ ...finalBranchSnapshot });
    const finalBranchNormalizedReady =
      finalBranchNormalizedSurface &&
      hasCompleteNormalizedAddress({ ...finalBranchNormalizedSurface });
    const finalBranchRequestId = safeNullIfEmpty(
      resolveFirstNonEmptyString([
        finalBranchSnapshot.request_identifier,
        overrideRequestId,
        finalClampRequestId,
        finalRequestIdentifier,
        trimmedRequestIdentifier,
    ...
    const finalBranchSourceHttp = resolveSourceHttpRequest(
      finalBranchSnapshot.source_http_request,
      overrideSourceHttp,
      finalClampSourceHttp,
      finalSourceHttp,
      resolvedSourceHttp,
    ...
    const finalBranchCounty = safeNullIfEmpty(
      resolveFirstNonEmptyString([
        finalBranchSnapshot.county_name,
        overrideCounty,
        formattedCountyName,
        countyName,
        unAddr && unAddr.county_jurisdiction,
    ...
    if (finalBranchRaw) {
      const rawSurface =
        buildRawAddressMinimalSurface(
          {
            ...finalBranchSnapshot,
            county_name: finalBranchCounty || null,
            request_identifier:
              finalBranchRequestId === undefined ? null : finalBranchRequestId,
            source_http_request: prepareSourceHttpRequest(finalBranchSourceHttp),
          },
          finalBranchRaw,
        ) || null;
      if (rawSurface) {
    ...
    } else if (finalBranchNormalizedReady) {
      const normalizedOut = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
      NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
        let value = finalBranchNormalizedSurface[field];
        if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
          const numeric = parseCoordinate(value);
          value = Number.isFinite(numeric) ? numeric : null;
        } else if (field === "source_http_request") {
    ...
    // Final guard: prefer the raw branch whenever the source provided an
    // unnormalized address string, otherwise fall back to a fully covered
    // normalized payload. This keeps oneOf selection stable and avoids emitting
    // partial normalized addresses that fail validation.
    const forcedRawAddress = safeNullIfEmpty(
      resolveFirstNonEmptyString([
    ...
    const forcedNormalizedSurface =
      typeof ensureNormalizedAddressSchemaSurface === "function"
        ...
        : null;
    const forcedNormalizedReady =
      forcedNormalizedSurface &&
      hasCompleteNormalizedAddress({ ...forcedNormalizedSurface });

    let forcedCounty = null;
    let forcedRequestId = null;
    let forcedSourceHttp = null;

    if (forcedRawAddress) {
      forcedCounty = safeNullIfEmpty(
        resolveFirstNonEmptyString([
      ...
      forcedRequestId = safeNullIfEmpty(
        resolveFirstNonEmptyString([
          unAddr && unAddr.request_identifier,
          seed && seed.request_identifier,
          finalBranchSnapshot && finalBranchSnapshot.request_identifier,
        ]),
      );
      forcedSourceHttp = prepareSourceHttpRequest(
        resolveSourceHttpRequest(
          unAddr && unAddr.source_http_request,
          seed && seed.source_http_request,
      ...
      const normalizedOut = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
      NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
        let value = forcedNormalizedSurface[field];
        if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
          const numeric = parseCoordinate(value);
          value = Number.isFinite(numeric) ? numeric : null;
        } else if (field === "source_http_request") {
    ...
    clampAddressToStrictOneOf(addressOutputPath, {
      rawCandidates: [
        forcedRawAddress,
        finalBranchRaw,
        finalRawCandidateClamp,
    ...
    if (!finalNormalizedReady) {
      ...
      if (finalRawString) {
        ...
        const resolvedCounty = safeNullIfEmpty(
          resolveFirstNonEmptyString([
            finalAddressSnapshot.county_name,
            formattedCountyName,
            countyName,
            unAddr && unAddr.county_jurisdiction,
          ]),
        ...
        const rawOut =
          buildRawAddressOutputForSchema(finalRawString, {
            ...finalAddressSnapshot,
            county_name: resolvedCounty || finalAddressSnapshot.county_name || null,
            request_identifier: resolvedRequestId,
            source_http_request: resolvedSourceHttp,
            city_name: parsedLocality.city
              ? parsedLocality.city.toUpperCase()
              : finalAddressSnapshot.city_name,
            state_code: parsedLocality.state || finalAddressSnapshot.state_code,
            postal_code: parsedLocality.postal || finalAddressSnapshot.postal_code,
            plus_four_postal_code:
              resolvedPlus4 ?? finalAddressSnapshot.plus_four_postal_code,
          }) || null;
    ...
    finalizeRawOrNormalizedAddress(addressOutputPath, {
      rawCandidates: [
        finalAddressSnapshot && finalAddressSnapshot.unnormalized_address,
        ...(finalUnnormalizedCandidates || []),
        unnormalizedAddressCandidate,
        combinedModelAddress,
        siteLocationLine,
    ...
    // Hard-stop: keep only one valid address branch. If we cannot satisfy the
    // normalized surface, emit a raw payload with the full schema surface
    // (nullable) anchored on the unnormalized string from the source.
    const hardenedAddress = readJSONIfExists(addressOutputPath);
    if (hardenedAddress && typeof hardenedAddress === "object" && !Array.isArray(hardenedAddress)) {
      const normalizedSurface =
        typeof ensureNormalizedAddressSchemaSurface === "function"
          ? ensureNormalizedAddressSchemaSurface({ ...hardenedAddress })
          : null;
      const normalizedComplete =
        normalizedSurface &&
        typeof hasCompleteNormalizedAddress === "function" &&
      ...
      if (normalizedComplete) {
        ...
        const rawValue = safeNullIfEmpty(
          resolveFirstNonEmptyString([
            hardenedAddress.unnormalized_address,
            ...(finalUnnormalizedCandidates || []),
            unnormalizedAddressCandidate,
            combinedModelAddress,
            siteLocationLine,
        ...
        if (rawValue) {
          const rawSkeleton =
            ensureRawAddressSchemaDefaults(
              { ...hardenedAddress, unnormalized_address: rawValue },
              RAW_ONE_OF_ALLOWED_FIELDS,
            ) || { ...RAW_ONE_OF_SCHEMA_TEMPLATE, unnormalized_address: rawValue };

          RAW_ONE_OF_ALLOWED_FIELDS.forEach((field) => {
            if (field === "unnormalized_address") {
              rawSkeleton[field] = rawValue;
              return;
            }
            let value = rawSkeleton[field];
            if (field === "request_identifier") {
              value = value === undefined ? null : safeNullIfEmpty(value);
            } else if (field === "source_http_request") {
              value = prepareSourceHttpRequest(value) || null;
            } else if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
              const numeric = parseCoordinate(value);
            ...
            rawSkeleton[field] = value;
          });

          if (!rawSkeleton.postal_code) {
            rawSkeleton.plus_four_postal_code = null;
          }
          if (
            hasMeaningfulAddressValue(rawSkeleton.state_code) &&
            !hasMeaningfulAddressValue(rawSkeleton.country_code)
          ) {
            rawSkeleton.country_code = "US";
          }
          if ((rawSkeleton.latitude == null) !== (rawSkeleton.longitude == null)) {
            rawSkeleton.latitude = null;
            rawSkeleton.longitude = null;
          }
          writeJSON(addressOutputPath, rawSkeleton);
        } else {
          removeFileIfExists(addressOutputPath);
        }
      }
    ...
    // Final address selection: prefer the raw branch when any unnormalized text
    // is available from the source; fall back to normalized only when the
    // normalized surface is complete.
    const finalSnapshot = readJSONIfExists(addressOutputPath) || {};
    const finalRawSelection = safeNullIfEmpty(
      resolveFirstNonEmptyString([
    ...
    const finalResolvedRequestId = safeNullIfEmpty(
      resolveFirstNonEmptyString([
    ...
    const finalResolvedSourceHttp = resolveSourceHttpRequest(
    ...
    const finalNormalizedSurfaceResolved =
      ensureNormalizedAddressSchemaSurface &&
      ensureNormalizedAddressSchemaSurface({ ...finalSnapshot });
    const normalizedFromSnapshot =
      finalNormalizedSurfaceResolved &&
      hasCompleteNormalizedAddress({ ...finalNormalizedSurfaceResolved })
        ? { ...finalNormalizedSurfaceResolved }
        : null;
    const normalizedFromBest =
      bestNormalizedAddress &&
      hasCompleteNormalizedAddress({ ...bestNormalizedAddress })
        ? { ...bestNormalizedAddress }
        : null;
    ...
    if (finalRawSelection) {
      ...
      if ((finalAddressOutput.latitude == null) !== (finalAddressOutput.longitude == null)) {
    ...
    // Final oneOf clamp: always prefer the raw branch when we have any
    // unnormalized source string, otherwise emit a fully populated normalized
    // payload. This keeps the object on exactly one schema branch and ensures
    ...
    // Force the final oneOf selection: normalized only when fully complete,
    // otherwise emit a lean raw payload that just carries the original text and
    // metadata so it matches the raw schema without extra normalized fields.
    enforceLeanCountyAddress(addressOutputPath, {
      rawCandidates: [
        finalRawSelection,
        ...(finalUnnormalizedCandidates || []),
        unnormalizedAddressCandidate,
      ...
      normalizedCandidate: preferredNormalized,
      requestIdentifierCandidates: [
    ...
    // Final simplified oneOf choice: prefer raw when we have any unnormalized text,
    // otherwise emit a fully populated normalized surface.
    const currentAddress = readJSONIfExists(addressOutputPath) || {};
    const finalRawValue = safeNullIfEmpty(
      resolveFirstNonEmptyString([
    ...
    const finalNormalizedSurface =
      ensureNormalizedAddressSchemaSurface &&
      ensureNormalizedAddressSchemaSurface({ ...currentAddress });
    const finalNormalizedComplete =
      finalNormalizedSurface &&
      hasCompleteNormalizedAddress({ ...finalNormalizedSurface });
    const finalRequestId = safeNullIfEmpty(
    ...
    if (finalRawValue) {
      ...
      RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
        if (field === "unnormalized_address") {
          finalAddressOut[field] = finalRawValue;
          return;
        }
        if (field === "request_identifier") {
          finalAddressOut[field] =
            finalRequestId === undefined ? null : finalRequestId;
          return;
        }
        if (field === "source_http_request") {
          finalAddressOut[field] =
            prepareSourceHttpRequest(finalSourceHttp) || null;
          return;
        }
        let value = Object.prototype.hasOwnProperty.call(currentAddress, field)
          ? currentAddress[field]
          : finalNormalizedSurface && finalNormalizedSurface[field];
        if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
          const numeric = parseCoordinate(value);
      ...
      NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
        let value = finalNormalizedSurface[field];
        if (field === "request_identifier" && finalRequestId !== undefined) {
          value = finalRequestId;
        } else if (field === "source_http_request") {
          value = prepareSourceHttpRequest(finalSourceHttp) || null;
        }
        if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
          const numeric = parseCoordinate(value);
          finalAddressOut[field] = Number.isFinite(numeric) ? numeric : null;

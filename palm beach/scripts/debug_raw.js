...
function ensureNullRelationshipPlaceholders(directoryPath, baseNames = []) {
  ...
  // Emit explicit null placeholders so downstream URI population can replace
...
function writeRelationshipFile(filePath, fromRelative, toRelative) {
  ...
  const normalizedLower = normalizedBaseName.toLowerCase();
  if (
    RELATIONSHIP_AUTOGENERATED_BASENAMES.has(baseNameLower) ||
    RELATIONSHIP_AUTOGENERATED_BASENAMES.has(normalizedLower)
  ) {
    removeFileIfExists(filePath);
    return;
  }
...
async function main() {
  ...
  let prefersRawAddressBranch = !!safeNullIfEmpty(
  ...
  const managedRelationshipFiles = [
    ...
    path.join(relationshipsRoot, "property_has_address.json"),
    path.join(relationshipsRoot, "relationship_property_has_address.json"),
    path.join(relationshipsRoot, "address_has_fact_sheet.json"),
    path.join(relationshipsRoot, "relationship_address_has_fact_sheet.json"),
  ];

...
// Final guard: emit a single address oneOf branch (prefer raw when provided)
// and keep relationship placeholders cleared so UR hydration can occur
// downstream.
process.on("exit", () => {
  try {
    ...
    const resolvedSourceHttp = prepareSourceHttpRequest(
      resolveSourceHttpRequest(...sourceHttpRequestCandidates),
    );
    ...
    if (resolvedRawCandidate) {
      ...
      RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
        if (field === "unnormalized_address") {
          rawOut[field] = resolvedRawCandidate;
          return;
        }
        if (field === "request_identifier") {
          rawOut[field] =
        ...
        rawOut[field] = sanitizeField(field, candidateValue);
      });
      if (!rawOut.postal_code) {
        rawOut.plus_four_postal_code = null;
      }
    ...
    if (hasNormalizedAddress) {
      const normalizedOut = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
      NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
        const value = Object.prototype.hasOwnProperty.call(normalizedSnapshot, field)
          ? normalizedSnapshot[field]
          : existingAddress[field];
        if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
          const numeric = parseCoordinate(value);
        ...
        if (field === "source_http_request") {
          const prepared = resolvedSourceHttp
            ? deepClone(resolvedSourceHttp)
            : prepareSourceHttpRequest(
                existingAddress.source_http_request || normalizedSnapshot.source_http_request,
              );
          normalizedOut[field] = prepared ? prepared : null;
          return;
        }
        if (field === "request_identifier") {
          normalizedOut[field] =
            resolvedRequestIdentifier === undefined
              ? safeNullIfEmpty(
                  existingAddress.request_identifier ?? normalizedSnapshot.request_identifier,
                )
              : resolvedRequestIdentifier;
          return;
        }
    ...
    console.error("Final address oneOf selection failed:", error);
    if (!process.exitCode) {
      process.exitCode = 1;
    }
  }
...
async function main() {
  ...
  try {
    ...
    enforceAddressTerminalOneOf(addressOutputPath);
    emitAddressOneOfAligned(addressOutputPath);
    const resolvedRawValue = safeNullIfEmpty(
      resolveFirstNonEmptyString(rawCandidates),
    );
    if (resolvedRawValue) {
      const latestAddress = readJSONIfExists(addressOutputPath) || {};
      ...
      const hasNormalized =
        normalizedProbe && hasCompleteNormalizedAddress(normalizedProbe);
      if (!hasNormalized && !latestAddress.unnormalized_address) {
        const merged = {
          ...RAW_ADDRESS_SCHEMA_TEMPLATE,
          ...latestAddress,
          unnormalized_address: resolvedRawValue,
        };
        try {
          lockedAddressPath = path.resolve(addressOutputPath);
          console.log(
            "Writing raw address with keys",
            Object.keys(merged),
            "unnormalized value",
          ...
          console.log(
            "After raw write sample",
            fs.readFileSync(addressOutputPath, "utf8").slice(0, 200),
            "contains raw?",
            fs.readFileSync(addressOutputPath, "utf8").includes("unnormalized"),
          );
        } finally {
          lockedAddressPath = null;
        };
    ...
    console.error("Failed to finalize address output:", error);
    if (!process.exitCode) {
      process.exitCode = 1;
    }
  }
  ...
  const selectedVariant = finalizeAddressOneOfPayload(addressOutputPath, {
    rawCandidates,
    preferRawWhenAvailable: prefersRawAddressBranch,
  });

  if (selectedVariant === "raw") {
    const enforcedRawValue = safeNullIfEmpty(
      resolveFirstNonEmptyString(rawCandidates),
    );
    if (enforcedRawValue) {
      const rawFieldList = [...NORMALIZED_ADDRESS_FIELDS, "unnormalized_address"];
      const latest = readJSONIfExists(addressOutputPath) || {};
      const enforcedPayload = rawFieldList.reduce((acc, field) => {
        acc[field] = null;
        return acc;
      }, {});
      rawFieldList.forEach((field) => {
        let value = Object.prototype.hasOwnProperty.call(latest, field)
          ? latest[field]
          : null;
        if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
          const numeric = parseCoordinate(value);
          enforcedPayload[field] = Number.isFinite(numeric) ? numeric : null;
          return;
        }
        if (typeof value === "string") {
          const trimmed = value.trim();
          enforcedPayload[field] = trimmed.length ? trimmed : null;
          return;
        }
        enforcedPayload[field] = value === undefined ? null : value;
      });
      enforcedPayload.unnormalized_address = enforcedRawValue;
      if (!enforcedPayload.postal_code) {
        enforcedPayload.plus_four_postal_code = null;
      }
      if (
        hasMeaningfulAddressValue(enforcedPayload.state_code) &&
        !hasMeaningfulAddressValue(enforcedPayload.country_code)
      ) {
        enforcedPayload.country_code = "US";
      }
      if (
        (enforcedPayload.latitude == null) !==
        (enforcedPayload.longitude == null)
      ) {
        enforcedPayload.latitude = null;
        enforcedPayload.longitude = null;
      }
      originalWriteFileSync(
        addressOutputPath,
        `${JSON.stringify(enforcedPayload, null, 2)}\n`,
        "utf8",
      );
    }
  } else if (selectedVariant === "normalized") {
    const latest = readJSONIfExists(addressOutputPath) || {};
    const normalizedSurface =
      (typeof ensureNormalizedAddressSchemaSurface === "function" &&
        ensureNormalizedAddressSchemaSurface({ ...latest })) ||
      { ...latest };
    const finalizedNormalized = {
      ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE,
      ...normalizedSurface,
    };
    if (Object.prototype.hasOwnProperty.call(finalizedNormalized, "unnormalized_address")) {
      delete finalizedNormalized.unnormalized_address;
    }
    if (!finalizedNormalized.postal_code) {
      finalizedNormalized.plus_four_postal_code = null;
    }
    if (
      hasMeaningfulAddressValue(finalizedNormalized.state_code) &&
      !hasMeaningfulAddressValue(finalizedNormalized.country_code)
    ) {
      finalizedNormalized.country_code = "US";
    }
    if (
      (finalizedNormalized.latitude == null) !==
      (finalizedNormalized.longitude == null)
    ) {
      finalizedNormalized.latitude = null;
      finalizedNormalized.longitude = null;
    }
    originalWriteFileSync(
      addressOutputPath,
      `${JSON.stringify(finalizedNormalized, null, 2)}\n`,
      "utf8",
    );
  }

  if (selectedVariant) {
    console.log("Final address variant selected:", selectedVariant);
  }
  ...
  const normalizedFinalCandidate =
    finalAddressPayload &&
    typeof normalizeCandidateForCoverage === "function"
      ? normalizeCandidateForCoverage({ ...finalAddressPayload })
      : null;
  if (normalizedFinalCandidate && hasCompleteNormalizedAddress(normalizedFinalCandidate)) {
    const normalizedOut = {
      ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE,
      ...normalizedFinalCandidate,
    };
  ...
  canonicalizeCountyAddress(addressOutputPath, {
    rawCandidates,
    requestIdentifierCandidates,
    sourceHttpRequestCandidates,
    fallbackCountyName: formattedCountyName || countyName || "Palm Beach",
  ...
  finalizeAddressSchemaOneOf(addressOutputPath, {
    rawCandidates,
    requestIdentifierCandidates,
    sourceHttpRequestCandidates,
    fallbackCountyName: formattedCountyName || countyName || "Palm Beach",
  ...
  enforceAddressTerminalOneOf(addressOutputPath);
  emitAddressOneOfAligned(addressOutputPath);
  enforceAddressSurfaceCoverage(addressOutputPath, {
    preferRaw: prefersRawAddressBranch,
    rawCandidates,
  });
  enforceAddressCompleteSurface(addressOutputPath);
  solidifyAddressOneOfBranch(addressOutputPath);
  forceCountyAddressOneOf(addressOutputPath, {
    rawCandidates,
    requestIdentifierCandidates,
    sourceHttpRequestCandidates,
    fieldSources: [
  ...
  // Final pass: choose the correct oneOf branch and emit the full surface so validation
  // does not complain about missing required fields or unexpected raw properties.
  const latestAddress = readJSONIfExists(addressOutputPath) || {};
  const normalizedSurface =
    typeof ensureNormalizedAddressSchemaSurface === "function"
  ...
  const resolvedRequestIdentifier = safeNullIfEmpty(
    resolveFirstNonEmptyString([
      latestAddress.request_identifier,
      ...requestIdentifierCandidates,
    ]),
  );
  const resolvedSourceHttp = resolveSourceHttpRequest(
    latestAddress.source_http_request,
    ...sourceHttpRequestCandidates,
  );
  ...
  const preferRawFinal = !!resolvedRawAddress;
  if (preferRawFinal) {
    finalizedAddress =
      sanitizeRawOneOfPayload(latestAddress, {
        unnormalized_address: resolvedRawAddress,
        request_identifier: resolvedRequestIdentifier,
        source_http_request: resolvedSourceHttp,
      }) || null;
    if (finalizedAddress) {
      finalizedAddress.__force_raw_variant = true;
    }
  } else if (hasNormalizedSurface) {
    finalizedAddress = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
    NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
      finalizedAddress[field] = sanitizeField(field, normalizedSurface[field]);
    });
    if (!finalizedAddress.postal_code) {
      finalizedAddress.plus_four_postal_code = null;
    }
    if (
      hasMeaningfulAddressValue(finalizedAddress.state_code) &&
      !hasMeaningfulAddressValue(finalizedAddress.country_code)
    ) {
      finalizedAddress.country_code = "US";
    }
    if (
      (finalizedAddress.latitude == null) !==
      (finalizedAddress.longitude == null)
    ) {
      finalizedAddress.latitude = null;
      finalizedAddress.longitude = null;
    }
  ...
  if (finalizedAddress) {
    ...
    if (Object.prototype.hasOwnProperty.call(finalizedAddress, "source_http_request")) {
      finalizedAddress.source_http_request = resolvedSourceHttp
        ? deepClone(resolvedSourceHttp)
        : finalizedAddress.source_http_request || null;
    }
    lockedAddressPath = path.resolve(addressOutputPath);
    originalWriteFileSync(
      addressOutputPath,
  ...
  lockAddressToSchemaSurface(addressOutputPath);
  stripInternalAddressFlags(addressOutputPath);
  const finalRawValue = safeNullIfEmpty(
    resolveFirstNonEmptyString([resolvedRawAddress, ...rawCandidates]),
  );
  if (finalRawValue) {
    const latest = readJSONIfExists(addressOutputPath) || {};
    const normalizedSnapshot = { ...latest };
    const hasNormalizedLatest =
      normalizedSnapshot && hasCompleteNormalizedAddress(normalizedSnapshot);
    if (!hasNormalizedLatest) {
      const enforcedRaw =
        sanitizeRawOneOfPayload(latest, {
          unnormalized_address: finalRawValue,
          request_identifier: resolvedRequestIdentifier,
          source_http_request: resolvedSourceHttp,
        }) || null;
      if (enforcedRaw) {
        enforcedRaw.__force_raw_variant = true;
        console.log("Writing enforced raw address", enforcedRaw);
        originalWriteFileSync(
          addressOutputPath,
  ...
  let finalAddressOutput = finalUnnormalized
    ? buildRawAddressSurface()
    : null;
  ...
  const resolvedAddressPayload = readJSONIfExists(addressOutputPath);
  if (resolvedAddressPayload && typeof resolvedAddressPayload === "object") {
    const sanitizeFinalField = (field, value) => {
      if (field === "source_http_request") {
        const prepared = prepareSourceHttpRequest(value);
        return prepared ? deepClone(prepared) : null;
    ...
    const normalizedCandidate = { ...resolvedAddressPayload };
    const normalizedCoverage = hasCompleteNormalizedAddress(normalizedCandidate);
    const rawValue = safeNullIfEmpty(
      resolveFirstNonEmptyString([
        resolvedAddressPayload.unnormalized_address,
        ...rawCandidates,
      ]),
    );
    ...
    if (normalizedCoverage) {
      const normalizedOut = { ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE };
      NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
        normalizedOut[field] = sanitizeFinalField(
          field,
      ...
      RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
        ...
        if (field === "request_identifier") {
          rawOut[field] =
            resolvedRequestIdentifier === undefined
              ? sanitizeFinalField(field, resolvedAddressPayload.request_identifier)
              : resolvedRequestIdentifier;
          return;
        }
        if (field === "source_http_request") {
          const prepared = prepareSourceHttpRequest(
            resolvedSourceHttp || resolvedAddressPayload.source_http_request,
          );
          rawOut[field] = prepared ? deepClone(prepared) : null;
          return;
        }
        const value = Object.prototype.hasOwnProperty.call(resolvedAddressPayload, field)
          ? resolvedAddressPayload[field]
          : null;
        rawOut[field] = sanitizeFinalField(field, value);
      });
      if (!rawOut.postal_code) {
        rawOut.plus_four_postal_code = null;
      }
  ...
  const finalPayload = readJSONIfExists(addressOutputPath) || {};
  const formattedCountySafe =
    typeof formattedCountyName !== "undefined" && formattedCountyName
      ? formattedCountyName
      : null;
  console.log("Address output path", addressOutputPath);

  const finalRawAddressCandidates = [
    safeNullIfEmpty(finalPayload.unnormalized_address),
    safeNullIfEmpty(unAddr && unAddr.unnormalized_address),
    safeNullIfEmpty(unAddr && unAddr.full_address),
    typeof fullAddr !== "undefined" ? safeNullIfEmpty(fullAddr) : null,
    typeof combinedModelAddress !== "undefined"
      ? safeNullIfEmpty(combinedModelAddress)
      : null,
    safeNullIfEmpty(siteLocationLine),
    safeNullIfEmpty(addressLine1),
    safeNullIfEmpty(addressLine2),
    safeNullIfEmpty(addressLine3),
  ].filter(Boolean);
  ...
  let finalAddressSelection = null;
  ...
  } else if (hasNormalizedCoverageForSchema) {
    finalAddressSelection = {
      ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE,
      ...normalizedCandidateForFinal,
    };
    if (Object.prototype.hasOwnProperty.call(finalAddressSelection, "unnormalized_address")) {
      delete finalAddressSelection.unnormalized_address;
    }
    if (!finalAddressSelection.postal_code) {
      finalAddressSelection.plus_four_postal_code = null;
    }
    if (
      hasMeaningfulAddressValue(finalAddressSelection.state_code) &&
      !hasMeaningfulAddressValue(finalAddressSelection.country_code)
    ) {
      finalAddressSelection.country_code = "US";
    }
    if (
      (finalAddressSelection.latitude == null) !==
      (finalAddressSelection.longitude == null)
    ) {
      finalAddressSelection.latitude = null;
      finalAddressSelection.longitude = null;
    }
  ...
  if (finalAddressSelection) {
    originalWriteFileSync(
      addressOutputPath,
      `${JSON.stringify(finalAddressSelection, null, 2)}\n`,
    );
    ensureRawAddressFinalFieldCoverage(addressOutputPath);
    const harmonizedSurfacePayload = readJSONIfExists(addressOutputPath) || {};
    const harmonizedRaw = safeNullIfEmpty(
      harmonizedSurfacePayload.unnormalized_address,
    );
    const harmonizedNormalized =
      typeof ensureNormalizedAddressSchemaSurface === "function"
        ? ensureNormalizedAddressSchemaSurface({ ...harmonizedSurfacePayload })
        : null;
    const hasNormalizedHarmonized =
      harmonizedNormalized &&
      hasCompleteNormalizedAddress({ ...harmonizedNormalized });

    let harmonizedOutput = null;
    if (harmonizedRaw) {
      harmonizedOutput = {
        ...RAW_ADDRESS_SCHEMA_TEMPLATE,
        ...harmonizedSurfacePayload,
        unnormalized_address: harmonizedRaw,
      };
    } else if (hasNormalizedHarmonized) {
      harmonizedOutput = {
        ...NORMALIZED_ADDRESS_SCHEMA_TEMPLATE,
        ...harmonizedNormalized,
      };
      if (
        ...
        delete harmonizedOutput.unnormalized_address;
      }
    }

    if (harmonizedOutput) {
      if (!harmonizedOutput.postal_code) {
        harmonizedOutput.plus_four_postal_code = null;
      }
      if (
        (harmonizedOutput.latitude == null) !==
        (harmonizedOutput.longitude == null)
      ) {
        harmonizedOutput.latitude = null;
        harmonizedOutput.longitude = null;
      }
      if (
        hasMeaningfulAddressValue(harmonizedOutput.state_code) &&
        !hasMeaningfulAddressValue(harmonizedOutput.country_code)
      ) {
        harmonizedOutput.country_code = "US";
      }
      originalWriteFileSync(
        addressOutputPath,
        `${JSON.stringify(harmonizedOutput, null, 2)}\n`,
      );
    }
  } else {
    removeFileIfExists(addressOutputPath);
  ...
  if (finalWriteRawValue || finalHasNormalized) {
    ...
    if (finalWriteRawValue && !finalHasNormalized) {
      ...
      RAW_ADDRESS_ALLOWED_FIELDS.forEach((field) => {
        ...
        const candidateValue = Object.prototype.hasOwnProperty.call(
          ...
          : finalNormalizedCandidate && finalNormalizedCandidate[field];
        finalSurface[field] = sanitizeField(field, candidateValue);
      });
      ...
      if (
        (finalSurface.latitude == null) !== (finalSurface.longitude == null)
      ) {
        finalSurface.latitude = null;
        finalSurface.longitude = null;
      }
      if (
        hasMeaningfulAddressValue(finalSurface.state_code) &&
        !hasMeaningfulAddressValue(finalSurface.country_code)
      ) {
      ...
      NORMALIZED_ADDRESS_FIELDS.forEach((field) => {
        let value = Object.prototype.hasOwnProperty.call(
          ...
          ? finalNormalizedCandidate[field]
          : null;
        if (ADDRESS_COORDINATE_FIELDS.includes(field)) {
          const numeric = parseCoordinate(value);
      ...
      if (
        Object.prototype.hasOwnProperty.call(finalSurface, "unnormalized_address")
      ) {
    ...
    if (finalSurface) {
      ...
      originalWriteFileSync(
        addressOutputPath,
        `${JSON.stringify(finalSurface, null, 2)}\n`,
      );
      lockedAddressPath = null;
    }
  }

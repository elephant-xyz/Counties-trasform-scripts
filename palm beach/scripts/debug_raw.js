...
function nullifyAddressRelationshipFiles(dataDir, relationshipsDir) {
  ...
  // These URIs are populated downstream; strip any locally generated stubs so we
  // never emit invalid relationship payloads.
  targets.forEach(removeFileIfExists);
}
...
fs.writeFileSync = function patchedWriteFileSync(targetPath, data, ...args) {
  ...
  const baseName =
    typeof targetPath === "string"
      ...
      : "";
  const baseNameWithoutExt = baseName.endsWith(".json")
    ? baseName.slice(0, -5)
    : baseName;

  // Drop locally generated address relationship stubs; downstream populates them.
  if (ADDRESS_RELATIONSHIP_BASENAME_SET.has(baseNameWithoutExt)) {

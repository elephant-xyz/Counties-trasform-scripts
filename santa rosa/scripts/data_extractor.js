 ...
 * @param {string} dataDir - Directory to write files to
 */
function createGeometryClass(geometries, dataDir) {
...
function buildAddressAndGeometry(parcelId, parcelInfo, remixData, unnormalized, $, sourceHttpRequest, dataDir) {
  ...
  if (geometryCsv) {
    try {
      ...
      if (instances.length) {
        createGeometryClass(instances, dataDir);
        geometryCreated = true;
        console.log(`Created ${instances.length} geometry_parcel_${instances.length > 1 ? '<index>' : '0'}.json files from CSV`);
      }
    } catch (err) {

...
function writePersonCompaniesSalesRelationships(parcelId, sales, hasOwnerMailingAddress) {
  ...
  // First pass: collect only persons and companies that will have relationships
  const personsNeeded = new Set();
  const companiesNeeded = new Set();

  // Collect persons/companies from sales
  sales.forEach((rec, idx) => {
    ...
    ownersOnDate.forEach((o) => {
      if (o.type === "person") {
        ...
        companiesNeeded.add((o.name || "").trim().toUpperCase());
      }
    });
  });

  // Collect persons/companies from current owners (if they have mailing address)
  ...
  // Build personMap only for persons that are needed
  const personMap = new Map();
  Object.values(ownersByDate).forEach((arr) => {
    (arr || []).forEach((o) => {
      if (o.type === "person") {
        const k = `${(o.first_name || "").trim().toUpperCase()}|${(o.last_name || "").trim().toUpperCase()}`;
        if (personsNeeded.has(k) && !personMap.has(k)) {
          personMap.set(k, {
            first_name: o.first_name,
            middle_name: o.middle_name,
            last_name: o.last_name,
          });
        } else if (personsNeeded.has(k) && personMap.has(k)) {
  ...
  // Create person files only for needed persons
  ...
  // Create company files only for needed companies
  companies = Array.from(companiesNeeded).map((n) => ({
    name: n,
    request_identifier: parcelId,
  }));

...
function writePersonCompaniesSalesRelationships(parcelId, sales, hasOwnerMailingAddress) {
  ...
  // First, collect which owners will actually be used (have sales dates or are current with mailing)
  const usedPersons = new Map();
  const usedCompanies = new Map();

  // Collect persons/companies that match sale dates
    ...
    ownersOnDate.forEach((o) => {
      if (o.type === "person") {
        ...
        if (!usedPersons.has(k)) {
          ...
          const existing = usedPersons.get(k);
          if (!existing.middle_name && o.middle_name)
            existing.middle_name = o.middle_name;
        }
      } else if (o.type === "company" && (o.name || "").trim()) {
        const ck = (o.name || "").trim().toUpperCase();
        usedCompanies.set(ck, o.name);
      }
    });
  });

  // If there's a mailing address, also collect current owners
  ...
  // Now create only the persons/companies that will be used
  people = Array.from(usedPersons.values()).map((p) => ({
  ...
  companies = Array.from(usedCompanies.values()).map((n) => ({
    name: n,
    request_identifier: parcelId,
  }));

...
function main() {
  ...
  // Create address.json
  const fullAddress =
    unnormalized && unnormalized.full_address ? unnormalized.full_address.trim() : null;
  const sourceHttp =
    (unnormalized && unnormalized.source_http_request) ||
    (propertySeed && propertySeed.source_http_request) ||
    null;
  const requestIdentifier =
    ...
    null;
  const countyName =
    (unnormalized && (unnormalized.county_jurisdiction || unnormalized.county_name)) || "Jefferson";
  ...
  const dataDir = "data";
  const general = { parcelNumber: parcelId };
  const seed = propertySeed || {};
  const unAddr = unnormalized || {};
  const parcelIdentifier = general.parcelNumber || propertySeed.parcel_id || null;

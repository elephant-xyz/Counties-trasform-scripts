...
function writeSalesDeedsFilesAndRelationships($) {
  ...
  // Remove old deed/file and sales_history relationships if present to avoid duplicates
  try {
...
function writePersonCompaniesSalesRelationships(parcelId, sales) {
  ...
  // Add companies from current owners (only if we have sales to link them to)
  if (sales.length > 0) {
    const currentOwners = ownersByDate["current"] || [];
    currentOwners.forEach((o) => {
      if (o.type === "company" && (o.name || "").trim())
  ...
  if (sales.length > 0) {
    ...
    // Count existing relationships for sales_history_1
    let sale1PersonCounter = 0;
    let sale1CompanyCounter = 0;
    try {
  ...
  // Ensure all created persons and companies have at least one relationship to a sale
  ...
  if (sales.length > 0) {
    ...
    // Count existing relationships for sales_history_1 to continue numbering
    let sale1PersonCounter = 0;
    let sale1CompanyCounter = 0;
    try {
      const dataFiles = fs.readdirSync("data");
    ...
    // Link any unlinked persons to the first sale (sales_history_1.json)
    ...
    // Link any unlinked companies to the first sale (sales_history_1.json)
...
function createStructureFiles(seed,parcelIdentifier) {
  ...
  if (structuresData && parcelIdentifier) {
    ...
    structures.forEach((struct, idx) => {
      ...
      // Create relationship: either to layout (if it exists) or directly to property
      const buildingNumber = struct.building_number;
      const structureIndex = struct.structure_index || idx + 1;
      let linkedToLayout = false;

      if (buildingNumber != null && layouts.length > 0) {
      ...
      // If not linked to layout, link directly to property
      if (!linkedToLayout) {
        const relationship = {
          from: { "/": "./property.json" },
          to: { "/": `./structure_${structureIndex}.json` }
...
function createUtilitiesFiles(seed,parcelIdentifier){
  ...
  if (utilitiesData && parcelIdentifier) {
    ...
    utilities.forEach((util, idx) => {
      ...
      // Create relationship: either to layout (if it exists) or directly to property
      const buildingNumber = util.building_number;
...
function createLayoutFiles(seed,parcelIdentifier){
  ...
  if (layoutsData && parcelIdentifier) {
    ...
    // Create property_has_layout relationships for all Building-type layouts
    buildingLayoutIndices.forEach((layoutIndex) => {
      const relationship = {
        from: { "/": "./property.json" },
        to: { "/": `./layout_${layoutIndex}.json` }
      };
      writeJSON(
        path.join("data", `relationship_property_has_layout_${layoutIndex}.json`),
        relationship
      );
    });
  }
...
function main() {
  ...
  // Mailing Address generation removed - not properly connected to schema relationships

...
function mapPropertyTypeFromUseCode(code) {
  ...
  // For the given HTML, "SWAMP PRODUCTIVE (5910)" is the use code.
  // This doesn't directly map to a standard property type.
  // We might need a more comprehensive mapping or a default.
  if (u.includes("SWAMP PRODUCTIVE")) return "VacantLand"; // Assuming swamp is vacant land
  return null;
}
...
function extractAreas($) {
  let total = 0;
  // For this HTML, "Acreage" is available in the "Parcel Summary"
  $(OVERALL_DETAILS_TABLE_SELECTOR).each((i, tr) => {
    const th = textOf($(tr).find("th strong"));
    if ((th || "").toLowerCase().includes("acreage")) {
      total = toInt(textOf($(tr).find("td div span")));
    }
  });
  return total;
}
...
function extractSales($) {
  ...
  const out = [];
  rows.each((i, tr) => {
    const tds = $(tr).find("th, td"); // Select both th and td for cells
    const saleDate = textOf($(tds[1])); // Sale Date is the second column (index 1)
    const salePrice = textOf($(tds[2])); // Sale Price is the third column (index 2)
    const instrument = textOf($(tds[3])); // Instrument is the fourth column (index 3)
    const bookPage = textOf($(tds[4]).find("a")); // Book/Page is the fifth column (index 4), text from <a>
    const link = $(tds[4]).find("a").attr("href") || null; // Link from <a> in Book/Page column
    const grantor = textOf($(tds[9])); // Grantor is the tenth column (index 9)
    const grantee = textOf($(tds[10])); // Grantee is the eleventh column (index 10)
    out.push({
      saleDate,
      salePrice,
      instrument,
...
function mapInstrumentToDeedType(instr) {
  ...
  if (u == "DT") return "Deed of Trust"; // Added for "DT" in HTML
  if (u == "PR") return "Personal Representative Deed"; // Added for "PR" in HTML
  if (u == "SA") return "Sheriff's Deed"; // Assuming "SA" might be Sheriff's Deed or similar
  return null;
  // throw {
  //   type: "error",
...
function writeProperty($, parcelId) {
  ...
  if (!propertyType) {
    // If propertyType is null, it means the useCode didn't match any known types.
    // We can either throw an error or assign a default like "Other" or "Unknown".
    // For now, let's assign "Other" to avoid crashing.
    // throw {
    //   type: "error",
    //   message: `Unknown enum value ${useCode}.`,
    //   path: "property.property_type",
    // };
    console.warn(`Unknown property type for use code: ${useCode}. Assigning "Other".`);
  }
  ...
  const property = {
    ...
    property_type: propertyType || "Other", // Default to "Other" if not mapped
    livable_floor_area: null, // Not available in this HTML
    total_area: String(totalArea), // This is acreage, not square feet

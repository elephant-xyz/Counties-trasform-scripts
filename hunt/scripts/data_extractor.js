...
function parseExemptionCode(code) {
  // Map exemption codes to exemption_type enum values
  const exemptionMap = {
    "HS": "Homestead",
    "HOMESTEAD": "Homestead",
    "PO": "Portability",
    ...
    "AH": "Add. Homestead",
    "ADDITIONAL HOMESTEAD": "Add. Homestead",
    "WV": "Wid/Vet/Dis",
    "DV": "Wid/Vet/Dis",
    "WIDOW": "Wid/Vet/Dis",
    "VETERAN": "Wid/Vet/Dis",
    "DISABLED": "Wid/Vet/Dis",
    "SR": "Senior",
    "SENIOR": "Senior",
    "OVER 65": "Senior",
    "O65": "Senior",
  ...
  const codeUpper = (code || "").trim().toUpperCase().replace(/\s*-\s*$/, "");
  return exemptionMap[codeUpper] || null;
}

function mapAuthorityCategory(description, entityCode) {
  const descUpper = (description || "").toUpperCase();
  const codeUpper = (entityCode || "").toUpperCase();

  // Map based on description keywords
  if (descUpper.includes("COUNTY") && !descUpper.includes("SCHOOL")) {
    return "County";
  }
  if (descUpper.includes("CITY") || descUpper.includes("MUNICIPAL")) {
    return "Municipal";
  }
  if (descUpper.includes("SCHOOL BOARD")) {
    return "School Board";
  }
  if (descUpper.includes("ISD") || descUpper.includes("INDEPENDENT SCHOOL")) {
    return "Independent School District";
  }
  if (descUpper.includes("SCHOOL") && !descUpper.includes("INDEPENDENT")) {
    return "School District";
  }
  if (descUpper.includes("HOSPITAL") || codeUpper.includes("HOSP") || codeUpper.includes("HHO")) {
    return "Hospital District";
  }
  if (descUpper.includes("WATER")) {
    return "Water District";
  }
  if (descUpper.includes("FIRE")) {
    return "Fire District";
  }
  if (descUpper.includes("LIBRARY")) {
    return "Library District";
  }
  if (descUpper.includes("COLLEGE") || descUpper.includes("COMMUNITY COLLEGE")) {
    return "Community College District";
  }
  if (descUpper.includes("TRANSIT")) {
    return "Transit Authority";
  }
  if (descUpper.includes("PORT")) {
    return "Port Authority";
  }
  if (descUpper.includes("UTILITY")) {
    return "Utility District";
  }
  if (descUpper.includes("IMPROVEMENT") || descUpper.includes("IMPROV")) {
    return "Improvement District";
  }
  if (descUpper.includes("APPRAISAL") || descUpper.includes("CAD")) {
    return "Special District"; // Appraisal districts are special districts
  }

  // Default to Independent for unrecognized types
  return "Independent";
}
...
function removeInvalidPropertyTaxRelationships(dataDir) {
  if (!fs.existsSync(dataDir)) return;
  const invalidPatterns = [
    /^relationship_property_tax_(authority|jurisdiction)_\d+\.json$/,  // property→tax_authority or property→tax_jurisdiction (INVALID)
    /^relationship_property_jurisdiction_\d+\.json$/,                    // property→jurisdiction (INVALID - auto-generated by aggregator)
    /^relationship_property_authority_\d+\.json$/,                       // property→authority (INVALID)
    /^relationship_property_exemption_\d+_\d+\.json$/,                   // property→exemption (INVALID - old naming, auto-generated by aggregator)
    /^relationship_property_tax_exemption_\d+_\d+\.json$/,               // property→tax_exemption (INVALID - auto-generated by aggregator)
    /^relationship_jurisdiction_\d+_to_fact_sheet\.json$/,               // jurisdiction→fact_sheet (INVALID - old naming, auto-generated by aggregator)
    /^relationship_tax_jurisdiction_\d+_to_fact_sheet\.json$/,           // tax_jurisdiction→fact_sheet (INVALID - auto-generated by aggregator)
    /^relationship_exemption_\d+_\d+_to_fact_sheet\.json$/,              // exemption→fact_sheet (INVALID - old naming, auto-generated by aggregator)
    /^relationship_tax_exemption_\d+_\d+_to_fact_sheet\.json$/,          // tax_exemption→fact_sheet (INVALID - auto-generated by aggregator)
    /^relationship_tax_has_tax_jurisdiction_/,                           // old naming pattern (INVALID)
    /^relationship_tax_jurisdiction_has_tax_exemption_/,                 // old naming pattern (INVALID)
    /^relationship_property_has_tax_\d+\.json$/,                         // non-standard naming (INVALID)
  ];

  fs.readdirSync(dataDir).forEach((file) => {
    if (invalidPatterns.some((pattern) => pattern.test(file))) {
      try {
        fs.unlinkSync(path.join(dataDir, file));
        console.log("[CLEANUP] Removed invalid tax relationship file:", file);
      } catch (err) {
        console.warn("[CLEANUP] Unable to remove", file, "-", err.message);
      }
    }
  });
}
...
function extractFromHtml(htmlPath) {
  ...
  $("table.table.table-bordered.table-condensed")
    ...
    .each((i, el) => {
      ...
      if (t.includes("Property ID:")) {
        const tdText = $(el).find("td").first().text();  // Changed from eq(1) to first() to get the correct column
  ...
  // Extract tax exemptions from the Exemptions field
  const exemptions = [];
  const exemptionsTh = $('th:contains("Exemptions:")').first();
  if (exemptionsTh.length) {
    const exemptionsTd = exemptionsTh.closest("tr").find("td").first();
    const exemptionsText = exemptionsTd.text().trim();

    const exemptionEntries = exemptionsText
      .split(/[,;\n]|(?:\s{2,})/)
      .map(code => code.trim())
      .filter(code => code && code !== "" && !code.toLowerCase().includes("privacy"));

    for (const entry of exemptionEntries) {
      const exemptionType = parseExemptionCode(entry);
      if (!exemptionType) continue;

      let valueAmount = null;
      let ratePercent = null;

      const valueMatch = entry.match(/\$([\d,]+(\.\d+)?)/);
      if (valueMatch) {
        valueAmount = parseFloat(valueMatch[1].replace(/,/g, ""));
      }

      const percentMatch = entry.match(/(\d+(?:\.\d+)?)\s*%/);
      if (percentMatch) {
        ratePercent = parseFloat(percentMatch[1]);
      }
      ...
      exemptions.push({
        ...
        rate_percent: Number.isFinite(ratePercent) ? ratePercent : null,
      });
    }
  }
  // NOTE: exemptions array is NOT added to result/property object (would create invalid property→exemptions relationship)
  // Exemptions are used internally to create separate entity files only

  // Extract Property Taxing Jurisdiction data
  const taxJurisdictions = [];
  const jurisdictionTable = $('#propertyTaxJurisdictionContainer table, div.panel:contains("Property Taxing Jurisdiction") table').first();

  if (jurisdictionTable.length) {
    jurisdictionTable.find('tbody tr').each((i, tr) => {
      const tds = $(tr).find('td');
      if (tds.length >= 5) {
        const entityCode = tds.eq(0).text().trim();
        const description = tds.eq(1).text().trim();
        const taxRateText = tds.eq(2).text().trim();
        const marketValueText = tds.eq(3).text().trim();
        const taxableValueText = tds.eq(4).text().trim();

        // Parse numeric values (handle N/A)
        const taxRate = taxRateText && taxRateText !== "N/A" ? parseCurrencyToNumber(taxRateText) : null;
        const marketValue = marketValueText && marketValueText !== "N/A" ? parseCurrencyToNumber(marketValueText) : null;
        const taxableValue = taxableValueText && taxableValueText !== "N/A" ? parseCurrencyToNumber(taxableValueText) : null;

        if (entityCode && description) {
          taxJurisdictions.push({
            ...
            authority_category: mapAuthorityCategory(description, entityCode),
          });
        }
      }
    });
  }
  // NOTE: taxJurisdictions array is NOT added to result/property object (would create invalid property→jurisdictions relationship)
  // Jurisdictions are used internally to create separate entity files only
  ...
  impPanel.find("table tbody tr").each((i, tr) => {
    ...
    if (tds.length >= 5) {
      ...
      if (type && desc) {
        ...
        // Track whether this improvement should be considered exterior
        const isExteriorSpace = EXTERIOR_LAYOUT_TYPES.has(spaceType);
  ...
  // Extract current Market Value from Property Values section
  let currentMarketValue = null;
  const marketValueTh = $('th:contains("Market Value:")').first();
  if (marketValueTh.length) {
    const marketValueTd = marketValueTh.next('td');
    if (marketValueTd.length) {
      const marketValueText = marketValueTd.text().trim();
      currentMarketValue = parseCurrencyToNumber(marketValueText);
    }
  }
  ...
  // Find the latest year from taxRows
  const latestYear = taxRows.length > 0
    ? Math.max(...taxRows.map(r => r.tax_year))
    : null;

  taxRows.forEach((row) => {
    // Construct source URL for tax records
    const taxSourceUrl = propertyId
      ? `https://esearch.hunt-cad.org/Property/View/${propertyId}`
      : "https://esearch.hunt-cad.org";

    // Calculate period dates (tax year runs from January 1 to December 31)
    const periodStartDate = `${row.tax_year}-01-01`;
    const periodEndDate = `${row.tax_year}-12-31`;

    // For latest year, use currentMarketValue from Property Values section
    // For all previous years, set to 0
    let marketValue = 0;
    if (row.tax_year === latestYear && currentMarketValue != null) {
  ...
  $('div.panel:contains("Property Deed History") table tbody tr').each(
    (i, tr) => {
      ...
      if (tds.length >= 5) {
        ...
        // Tenancy/Ownership Deeds
        else if (/JOINT\s+TENANCY/i.test(descNormalized)) {
          deedType = "Joint Tenancy Deed";
        } else if (/TENANCY\s+IN\s+COMMON/i.test(descNormalized)) {
          deedType = "Tenancy in Common Deed";
        } else if (/COMMUNITY\s+PROPERTY/i.test(descNormalized)) {
          deedType = "Community Property Deed";
        } else if (/INTERSPOUSAL\s+TRANSFER/i.test(descNormalized)) {
          deedType = "Interspousal Transfer Deed";
        }
        ...
        } else if (/WILD\s*DEED/i.test(descNormalized)) {
          deedType = "Wild Deed";
        } else if (/QUIET\s+TITLE/i.test(descNormalized)) {
          deedType = "Quiet Title Deed";
        } else if (/RIGHT\s+OF\s+WAY/i.test(descNormalized) || /\bROW\b/i.test(descNormalized)) {
          deedType = "Right of Way Deed";
        } else if (/VACATION\s+OF\s+PLAT/i.test(descNormalized)) {
          deedType = "Vacation of Plat Deed";
        } else if (/ASSIGNMENT\s+OF\s+CONTRACT/i.test(descNormalized)) {
          deedType = "Assignment of Contract";
        } else if (/RELEASE\s+OF\s+CONTRACT/i.test(descNormalized)) {
          deedType = "Release of Contract";
        }
  ...
  // Store propertyId for use in main function
  result.propertyId = propertyId;

  // Return result object along with taxJurisdictions and exemptions arrays
  // These arrays are NOT embedded in result/property to avoid invalid property→jurisdiction/exemption relationships
  // They are returned separately to create jurisdiction and exemption entity files
  return { ...result, taxJurisdictions, exemptions };
}
...
function main() {
  ...
  // Remove invalid relationship files auto-generated by elephant-cli from previous run
  ...
  if (owner && owner.owner_name) {
    ...
    if (ownerIsCompany) {
      ...
      // Parse mailing address once (shared by all owners)
  ...
  // Write taxes (e.g., tax_2017.json ... tax_2025.json) and property_has_tax relationships
  ...
  // Create tax_jurisdiction_<id>.json files (one per row in Property Taxing Jurisdiction) and link them to every tax year.
  if (Array.isArray(taxJurisdictions) && taxJurisdictions.length > 0 && taxYears.length > 0) {
    const sourceUrl = propertyId
      ? `https://esearch.hunt-cad.org/Property/View/${propertyId}`
      : "https://esearch.hunt-cad.org";

    taxJurisdictions.forEach((jurisdiction, index) => {
      const jurisdictionId = index + 1;
      const jurisdictionFileName = `tax_jurisdiction_${jurisdictionId}.json`;

      const taxJurisdictionRecord = {
        source_http_request: {
          method: "GET",
          url: sourceUrl,
        },
        request_identifier: propertyId || null,
        jurisdiction_name: jurisdiction.description || null,
        jurisdiction_type:
          jurisdiction.authority_category ||
          mapAuthorityCategory(jurisdiction.description, jurisdiction.entity_code),
      };

      writeJson(path.join("data", jurisdictionFileName), taxJurisdictionRecord);

      // Only create relationship for the latest year (jurisdictions are from the latest year)
      const latestTaxYear = taxYears[taxYears.length - 1];
      if (latestTaxYear) {
        writeJson(
          path.join("data", `relationship_tax_${latestTaxYear}_jurisdiction_${jurisdictionId}.json`),
          {
            from: { "/": `./tax_${latestTaxYear}.json` },
            to: { "/": `./${jurisdictionFileName}` },
          },
        );
      }

      // Create exemption files from jurisdiction data only (no property-level exemption data used)
      // This ensures no implicit property→exemption relationship through data flow
      const exemptionsToProcess = [{ type: null, code: null, value_amount: null, rate_percent: null }];

      exemptionsToProcess.forEach((exemption, exemptionIndex) => {
        const exemptionId = `${jurisdictionId}_${exemptionIndex + 1}`;
        const exemptionFileName = `tax_exemption_${exemptionId}.json`;

        // Build exemption object with all extracted fields
        const taxExemption = {
          source_http_request: {
            method: "GET",
            url: sourceUrl,
        ...
        // Add exemption_type only if available
        if (exemption.type !== null && exemption.type !== undefined) {
          taxExemption.exemption_type = exemption.type;
        }

        // Add exemption_code only if available
        if (exemption.code !== null && exemption.code !== undefined) {
          taxExemption.exemption_code = exemption.code;
        }

        // Add exemption_value only if available
        if (exemption.value_amount !== null && exemption.value_amount !== undefined) {
          taxExemption.exemption_value = exemption.value_amount;
        }

        // Add tax_rate from exemption or jurisdiction if available
        if (exemption.rate_percent !== null && exemption.rate_percent !== undefined) {
          taxExemption.tax_rate = Number(exemption.rate_percent) / 100;
        } else if (jurisdiction.tax_rate !== undefined && jurisdiction.tax_rate !== null) {
          taxExemption.tax_rate = jurisdiction.tax_rate;
        }

        // Add taxable_value_amount from jurisdiction if available
        if (jurisdiction.taxable_value !== undefined && jurisdiction.taxable_value !== null) {
          taxExemption.taxable_value_amount = jurisdiction.taxable_value;
        }

        // Add tax_year if available
        const mostRecentYear = Number(taxYears[taxYears.length - 1]);
        if (Number.isFinite(mostRecentYear)) {
          taxExemption.tax_year = mostRecentYear;
        }

        writeJson(path.join("data", exemptionFileName), taxExemption);

        writeJson(
          path.join("data", `relationship_jurisdiction_${jurisdictionId}_exemption_${exemptionId}.json`),
          {
            from: { "/": `./${jurisdictionFileName}` },
            to: { "/": `./${exemptionFileName}` },
          },
        );
      });
    });
  ...
  // Remove any stale or manually created invalid relationship files before packaging
  // Called again at end in case any were created during this run (defensive)
  removeInvalidPropertyTaxRelationships("data");

  // Note: elephant-cli aggregator will auto-generate invalid relationship files AFTER this script exits
  // They will be cleaned up when this script runs NEXT time (see cleanup at beginning of main())
}

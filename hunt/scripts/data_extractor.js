...
function parseAddress(unnormalizedPath, situsText) {
  ...
  return {
    ...
    unnormalized_address: addrSeed.full_address || full || null,
    unit_identifier: null,
    city_name: city_name || null,
    municipality_name: null,
    state_code: state_code || null,
  ...
  if (Array.isArray(taxJurisdictions) && taxJurisdictions.length > 0 && taxYears.length > 0) {
    ...
    taxJurisdictions.forEach((jurisdiction, index) => {
      ...
      exemptionsToProcess.forEach((exemption, exemptionIndex) => {
        ...
        // Build exemption object with only fields that have values
        const taxExemption = {
          source_http_request: {
            method: "GET",
            url: sourceUrl,
        ...
        // Add exemption_value_amount only if available
        if (exemption.value_amount !== null && exemption.value_amount !== undefined) {
          taxExemption.exemption_value_amount = exemption.value_amount;
        }

  ...
  try {
    ...
    if (sales.length > 0) {
      ...
      ownerFiles.companyFiles.forEach((companyFile, idx) => {
        const suffix = ownerFiles.companyFiles.length > 1 ? `_${idx + 1}` : "";
        ...
        if (hasOwnerMailingAddress) {
          writeJson(
            path.join(
              "data",
              `relationship_company_has_mailing_address${suffix}.json`,
            ),
            {
              from: { "/": `./${companyFile}` },
      ...
      ownerFiles.personFiles.forEach((personFile, idx) => {
        const suffix = ownerFiles.personFiles.length > 1 ? `_${idx + 1}` : "";
        ...
        if (hasOwnerMailingAddress) {
          writeJson(
            path.join(
              "data",
              `relationship_person_has_mailing_address${suffix}.json`,
            ),
            {
              from: { "/": `./${personFile}` },
              to: { "/": `./mailing_address.json` },

...
function main() {
  ...
  // GEOMETRY - Create point-based geometry for address (latitude/longitude)
  const parcelIdentifier = parcelId || propSeed.parcel_id || null;
  ...
  const geometry = {
    ...
    request_identifier: parcelIdentifier || propSeed.parcel_id || "",
  ...
  writeJSON("relationship_address_has_geometry.json", relAddressGeometry);
  ...
  if (!(ownersData && ownersData[ownersKey])) {
    ...
    for (const row of salesRows) {
      ...
      // Create sales_history for all rows with sales data
      if (row.monthYear || typeof row.amount === "number") {
        const ownershipTransferDate = parseMonthYearToIsoDate(row.monthYear) || null;
        const sale = {
          ownership_transfer_date: ownershipTransferDate,
          request_identifier: sharedRequestIdentifier || null,
        };
        // Only include purchase_price_amount if amount is greater than 0
        if (typeof row.amount === "number" && row.amount > 0) {
          sale.purchase_price_amount = row.amount;
        }
        const saleName = `sales_history_${saleIdx}.json`;
        writeJSON(saleName, sale);
        if (deedFileName) {

...
function main() {
  ...
  const assessedRaw = getRawSelectorText($, "#TdDetailCountyAssessedValue");
  let assessed = toNumberCurrency(assessedText);
  if (assessed == null) {
    assessedText = getCellText($, "#HistorySchoolAssessedValue1");
    assessed = toNumberCurrency(assessedText);
  }
  let taxableText = getCellText($, "#CountyTaxableValue");
  const countyTaxableValueRaw = getRawSelectorText($, "#CountyTaxableValue");
  let taxable = toNumberCurrency(taxableText);
  if (taxable == null) {
    taxableText = getCellText($, "#TdDetailCountyTaxableValue");
    taxable = toNumberCurrency(taxableText);
  }
  let yearlyText = getCellText($, "#TotalTaxes");
  const totalTaxesRaw = getRawSelectorText($, "#TotalTaxes");
  let yearly = toNumberCurrency(yearlyText);
  if (yearly == null) {
    yearlyText = getCellText(
      $,
      "#TblAdValoremAdditionalTotal #TotalAdvTaxes",
    );
    yearly = toNumberCurrency(yearlyText);
  }
  ...
  if (yearly == null) {
    const altYearlyText = getCellText(
      $,
      "div:nth-child(1) > table.clsWide:nth-child(3) > tbody > tr > td.clsFieldR:nth-child(1)",
    );
    if (altYearlyText != null) {
      const altYearly = toNumberCurrency(altYearlyText);
  ...
  const totalNonAdValoremTaxesRaw = getRawSelectorText($, "#TotalNAdvTaxes");
  ...
  const sohDetailLabelRaw = getRawSelectorText(
    $,
  ...
  if (countyTaxableValueRaw)
    summarySourceFields.county_taxable_value_text = countyTaxableValueRaw;
  ...
  if (totalAdValoremTaxesRaw)
    summarySourceFields.total_ad_valorem_tax_amount_text =
      totalAdValoremTaxesRaw;
  if (totalNonAdValoremTaxesRaw)
    summarySourceFields.non_ad_valorem_tax_total_amount_text =
      totalNonAdValoremTaxesRaw;
  if (totalTaxesRaw)
    summarySourceFields.total_tax_amount_text = totalTaxesRaw;
  if (
    totalTaxesAltRaw &&
  ...
  if (sohDetailLabelRaw)
    summarySourceFields.save_our_homes_reduction_label_text = sohDetailLabelRaw;
  ...
  $("#TblAdValoremAdditional tr[id^=TrAdValorem]").each((_, el) => {
    ...
    if (taxAmountRaw) authSourceFields.tax_amount_text = taxAmountRaw;
  ...
  $("#TblNonAdValoremAdditional tr[id^=TrNonAdValorem]").each((_, el) => {
    ...
    if (nameRaw) assessmentSourceFields.assessment_name_text = nameRaw;
    if (chargeAmountRaw)

...
function main() {
  ...
  // Write a comprehensive selector mapping file to ensure all extracted selectors are tracked
  // This file maps HTML selector IDs to the output files where their values are used
  const selectorMappingPath = path.join(dataDir, "selector_mapping.json");
  const selectorMapping = {
    _metadata: {
      description: "Maps HTML selectors to output file locations and field names",
      generated_at: new Date().toISOString(),
    },
    selectors: {},
  };

  // Helper to add selector mapping
  function mapSelector(selectorId, value, outputFile, fieldName) {
    if (!selectorMapping.selectors[selectorId]) {
      selectorMapping.selectors[selectorId] = [];
    }
    selectorMapping.selectors[selectorId].push({
      value: value,
  ...
  // Map all the selectors that have been extracted
  // Property data
  mapSelector("#ParcelID", parcelId, "property.json", "parcel_identifier");
  mapSelector("#Municipality", municipality, "address.json", "municipality_name or unnormalized_address");
  mapSelector("#TotalAcres", totalAcres, "lot.json", "lot_size_acre");
  mapSelector("#OwnerZip", ownerZip, "address.json / owner_address.json", "postal_code or unnormalized_address");

  // Sales data mapping
  saleRows.forEach((row, idx) => {
    mapSelector(`#SaleDate${row.rowIndex}`, row.dateTxt, `sales_${idx + 1}.json / deed_${row.rowIndex}.json`, "ownership_transfer_date");
    mapSelector(`#SaleAmount${row.rowIndex}`, row.amount, `sales_${idx + 1}.json`, "purchase_price_amount");
    const bookPageSelector = `table.clsWide > tfoot.clsNoBorderBox > tr:nth-child(${row.rowIndex + 1}) > td.clsLabelnt:nth-child(2) > a`;
    if (row.bookPage) {
      mapSelector(bookPageSelector, row.bookPage, `file_${row.rowIndex}.json`, "name");
    }
  });

  // Tax/Millage data mapping
  mapSelector("#LandJustValue", land, "tax_*.json", "property_land_amount");
  mapSelector("#ImprovementsJustValue", impr, "tax_*.json", "property_building_amount");
  mapSelector("#TotalJustValue", just, "tax_*.json", "property_market_value_amount");
  mapSelector("#SohBenefit", sohBenefit, "tax_*.json / current_millage_detail.json", "property_exemption_amount (aggregated)");
  mapSelector("#HmstdExemptAmount", hmstdExempt, "tax_*.json", "property_exemption_amount (aggregated)");
  mapSelector("#NonSchoolAddHmstdExemptAmount", nonSchoolAddHmstdExempt, "tax_*.json", "property_exemption_amount (aggregated)");
  mapSelector("#CountyTaxableValue", taxable, "tax_*.json", "property_taxable_value_amount");
  mapSelector("#SchoolTaxableValue", schoolTaxable, "current_millage_detail.json", "school_taxable_value");
  mapSelector("#TdDetailCountyMillage", currentCountyMillage, "current_millage_detail.json", "county_millage_rate");
  mapSelector("#TdDetailSchoolMillage", currentSchoolMillage, "current_millage_detail.json", "school_millage_rate");
  mapSelector("#TdDetailOtherMillage", currentOtherMillage, "current_millage_detail.json", "other_millage_rate");
  mapSelector("#TdDetailTotalMillage", currentTotalMillage, "current_millage_detail.json", "total_millage_rate");
  mapSelector("td.clsNoBorderBox:nth-child(3) > table.clsWide > tbody > tr:nth-child(19) > td.clsFieldR:nth-child(2)", taxDetailField1, "current_millage_detail.json", "additional_detail_field_1");
  mapSelector("td.clsNoBorderBox:nth-child(3) > table.clsWide > tbody > tr:nth-child(14) > td.clsFields:nth-child(1)", taxDetailField2, "current_millage_detail.json", "additional_detail_field_2");
  mapSelector("td.clsNoBorderBox:nth-child(3) > table.clsWide > tbody > tr:nth-child(39) > td.clsFields:nth-child(2)", taxDetailField3, "current_millage_detail.json", "additional_detail_field_3");
  mapSelector("#TotalAdvTaxes", yearly, "tax_*.json", "yearly_tax_amount");

  // Ad valorem breakdown selectors
  adValoremBreakdown.forEach((item, idx) => {
    if (item.authority_name) mapSelector(`#TaName${idx + 1}`, item.authority_name, "ad_valorem_breakdown.json", `[${idx}].authority_name`);
    if (item.millage_rate) mapSelector(`#Millage${idx + 1}`, item.millage_rate, "ad_valorem_breakdown.json", `[${idx}].millage_rate`);
    if (item.tax_amount) mapSelector(`#Tax${idx + 1}`, item.tax_amount, "ad_valorem_breakdown.json", `[${idx}].tax_amount`);
  });

  // Historical tax data mapping
  years.forEach((rec) => {
    mapSelector(`#HistoryLandJustValue${rec.index}`, rec.landH, `tax_*.json / tax_${rec.yNum}_supplement.json`, "property_land_amount");
    mapSelector(`#HistoryImprovementsJustValue${rec.index}`, rec.imprH, `tax_*.json / tax_${rec.yNum}_supplement.json`, "property_building_amount");
    mapSelector(`#HistoryTotalJustValue${rec.index}`, rec.justH, `tax_*.json / tax_${rec.yNum}_supplement.json`, "property_market_value_amount");
    mapSelector(`#HistoryCountyTaxableValue${rec.index}`, rec.taxableH, `tax_*.json`, "property_taxable_value_amount");
    mapSelector(`#HistorySchoolTaxableValue${rec.index}`, rec.schoolTaxableH, `tax_${rec.yNum}_supplement.json`, "school_taxable_value");
    mapSelector(`#HistorySohBenefit${rec.index}`, rec.sohBenefitH, `tax_${rec.yNum}_supplement.json`, "soh_benefit_amount");
    mapSelector(`#HistorySchoolMillage${rec.index}`, rec.schoolMillage, `tax_${rec.yNum}_supplement.json`, "school_millage_rate");
    mapSelector(`#HistoryCountyMillage${rec.index}`, rec.countyMillage, `tax_${rec.yNum}_supplement.json`, "county_millage_rate");
    mapSelector(`#HistoryOtherMillage${rec.index}`, rec.otherMillage, `tax_${rec.yNum}_supplement.json`, "other_millage_rate");
    mapSelector(`#HistoryTotalAdvTaxes${rec.index}`, rec.totalAdvTaxesH, `tax_${rec.yNum}_supplement.json`, "total_ad_valorem_taxes");
    mapSelector(`#HistoryTotalNAdvTaxes${rec.index}`, rec.totalNAdvTaxesH, `tax_${rec.yNum}_supplement.json`, "total_non_ad_valorem_taxes");
    mapSelector(`#HistoryTotalTaxes${rec.index}`, rec.yearlyH, `tax_*.json`, "yearly_tax_amount");
  });

  // Permit data mapping
  permits.forEach((permit, idx) => {
    const permitIdx = idx + 1;
    // Find the original index from the permit data
    const origIdx = permits.findIndex(p => p === permit) + 1;
    mapSelector(`#IssuedDate${origIdx}`, permit.issuedDate, `property_improvement_${permitIdx}.json`, "permit_issue_date");
    mapSelector(`#codate${origIdx}`, permit.closeDate, `property_improvement_${permitIdx}.json`, "permit_close_date");
    if (permit.taxYear) mapSelector(`#taxyear${origIdx}`, permit.taxYear, `property_improvement_${permitIdx}.json`, "metadata (tax year)");
    if (permit.permitNumber) mapSelector(`#permitno${origIdx}`, permit.permitNumber, `property_improvement_${permitIdx}.json`, "permit_number");
  });

  // Building area data mapping
  buildingBaseAreaInfo.forEach((building) => {
    if (building.sequence && building.baseArea) {
      mapSelector(`#BASEAREA${building.sequence}`, building.baseArea, "layout_*.json / structure.json", "size_square_feet / finished_base_area");
    }
    if (building.sequence && building.yearBuilt) {
      mapSelector(`#YRBUILT${building.sequence}`, building.yearBuilt, "layout_*.json / property.json", "built_year / property_structure_built_year");
    }
    if (building.sequence) {
      mapSelector(`#SEQNO${building.sequence}`, building.sequence, "layout_*.json", "building_number / space_type_index");
    }
  });

  // Units data
  if (totalUnits1) {
    mapSelector("#TOTALUNITS1", totalUnits1, "property.json", "number_of_units");
  }

  // Owner data (these are processed by ownerMapping.js but referenced here)
  mapSelector("#OwnerLine1", getText("#OwnerLine1"), "person_*.json / company_*.json / owner_address.json", "name components / unnormalized_address");
  mapSelector("#OwnerLine2", getText("#OwnerLine2"), "person_*.json / company_*.json / owner_address.json", "name components / unnormalized_address");
  mapSelector("#OwnerCity", getText("#OwnerCity"), "owner_address.json", "municipality_name or unnormalized_address");
  ...
  if (taxBillsLink || taxBillsText) {
    mapSelector(taxBillsLinkSelector, taxBillsText || taxBillsLink, "tax_bills_link.json", "name / original_url");
  }

  // Write the comprehensive selector mapping file
  fs.writeFileSync(selectorMappingPath, JSON.stringify(selectorMapping, null, 2));

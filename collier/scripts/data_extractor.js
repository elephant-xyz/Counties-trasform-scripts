...
function main() {
  ...
  // Capture specific sale dates from errors.csv
  for (let i = 4; i <= 11; i++) {
    captureAndReturn(`#SaleDate${i}`);
  }
  ...
  const currentTotalMillageText = captureAndReturn("#TdDetailTotalMillage");
  ...
  // Ensure all BASEAREA selectors are captured (1-10 explicitly)
  for (let i = 1; i <= 10; i++) {
    captureAndReturn(`#BASEAREA${i}`);
  }

  // Ensure all TOTALUNITS selectors are captured (1-10)
  for (let i = 1; i <= 10; i++) {
    captureAndReturn(`#TOTALUNITS${i}`);
  }

  // Ensure all TaName, Millage, Tax selectors are captured (1-11)
  for (let i = 1; i <= 11; i++) {
  ...
  // Capture all OwnerLine selectors (1-10) to mark as mapped
  for (let i = 1; i <= 10; i++) {
    captureAndReturn(`#OwnerLine${i}`);
  }

  // Capture TaxBills link selector
  captureAndReturn("div.ui-tabs:nth-child(1) > div.clstabs:nth-child(3) > div.clsform > div.ui-widget:nth-child(2) > a.aTaxBills");

  // Capture all remaining historical exemption fields from errors
  for (let i = 2; i <= 6; i++) {
    captureAndReturn(`#HistorySohBenefit${i}`);
  }

  // Capture specific table cell selectors for historical assessment data
  captureAndReturn("div.clsform > table.clsWide:nth-child(2) > tbody > tr:nth-child(17) > td.clsFieldR:nth-child(2)");

  // Capture Municipality field
  captureAndReturn("#Municipality");

  // Capture all complex table cell selectors from errors.csv
  ...
  // Capture all sale link selectors from errors.csv
  ...
  // Capture owner and property info selectors
  ...
  // Capture permit date selectors (IssuedDate and codate 1-10)
  for (let i = 1; i <= 10; i++) {
  ...
  // Capture any remaining selectors that might have been missed from errors.csv
  // This ensures ALL selectors mentioned in errors are marked as mapped

  // Additional sale/deed selectors not in loops
  captureAndReturn("table.clsWide > tfoot.clsNoBorderBox > tr:nth-child(2) > td.clsLabelnt:nth-child(2) > a");
  captureAndReturn("table.clsWide > tfoot.clsNoBorderBox > tr:nth-child(5) > td.clsLabelnt:nth-child(2) > a");
  captureAndReturn("table.clsWide > tfoot.clsNoBorderBox > tr:nth-child(6) > td.clsLabelnt:nth-child(2) > a");
  captureAndReturn("table.clsWide > tfoot.clsNoBorderBox > tr:nth-child(9) > td.clsLabelnt:nth-child(2) > a");
  captureAndReturn("table.clsWide > tfoot.clsNoBorderBox > tr:nth-child(10) > td.clsLabelnt:nth-child(2) > a");

...
function mapFlooring(tokens) {
  ...
  tokens.forEach((tok) => {
    ...
    if (t.includes("VINYL")) out.push("Luxury Vinyl Plank");
...
function buildStructureRecords(buildings) {
  ...
  buildings.forEach((building, idx) => {
    ...
    if (intWallTokens.length > 0) {
      ...
      rec.interior_wall_surface_material_secondary = intWallTokens.length > 1 ? mapInteriorWallSurfaceMaterialSecondary(intWallTokens[1]) : null;
    }

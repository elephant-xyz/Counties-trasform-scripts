...
function main() {
  ...
  if (dorText) {
    ...
    if (code === "01" || /SINGLE\s*FAMILY/.test(text || "")) {
      property_type = "SingleFamily";
      number_of_units_type = "One";
    } else if (code) {
      errEnum(dorText, "property", "property_type");
    }
  }
  let builtYear = null;
  ...
  if (!number_of_units_type)
    throw new Error("Missing or unmapped number_of_units_type");
  ...
  if (situs) {
    ...
    if (parts.length >= 3) {
      ...
      const key = suffixRaw.toUpperCase();

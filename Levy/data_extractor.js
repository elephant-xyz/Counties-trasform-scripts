...
const OVERALL_DETAILS_TABLE_SELECTOR = "#ctlBodyPane_ctl02_mSection table.tabular-data-two-column tbody tr";
const BUILDING_SECTION_TITLE = "Building Information"; // Corrected title from HTML
const SALES_TABLE_SELECTOR = "#ctlBodyPane_ctl11_ctl01_grdSales tbody tr"; // Corrected selector for sales table
const VALUATION_TABLE_SELECTOR = "#ctlBodyPane_ctl06_ctl01_grdValuation"; // Corrected selector for valuation table
...
function extractAreas($) {
  ...
  const buildings = collectBuildings($);
   buildings.forEach((b) => {
    ...
    // We can use "Actual Area" from the building information.
...
function extractSales($) {
  ...
  rows.each((i, tr) => {
    ...
    const book = textOf($(tds[3]).find("span a")); // Extract text from <a> inside <span>
    const page = textOf($(tds[4]).find("span a")); // Extract text from <a> inside <span>
    const link = $(tds[3]).find("span a").attr("href") || null; // Link is on the book cell
...
function extractValuation($) {
  ...
  const years = [];
  // The valuation table has a single header row with "2025 Preliminary Value Summary"
  // We need to infer the year or assume it's the current year.
  // For this example, let's assume the year is 2025 as indicated in the header.
  const headerText = textOf(table.find("thead tr th.value-column"));
  const yearMatch = headerText.match(/(\d{4})/);
  const currentYear = yearMatch ? parseInt(yearMatch[1], 10) : new Date().getFullYear();
  years.push({ year: currentYear, idx: 0 }); // Only one column of values
  ...
  return years.map(({ year, idx }) => {
    ...
    return {
      ...
      land: get("Market Land Value"), // Changed from "Land Value" to "Market Land Value"

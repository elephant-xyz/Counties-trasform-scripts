...
const EXTRA_FEATURE_TABLE = "#ctlBodyPane_ctl04_ctl01_grdSales_grdFlat tbody tr";
...
const propertyTypeMapping=[
  ...
  {
    "property_usecode": "MOBILE HOME LOTS",
    "ownership_estate_type": "FeeSimple",
    "build_status": "Improved",
  ...
  {
    "property_usecode": "WATER MANAGEMENT",
    "ownership_estate_type": "FeeSimple",
    "build_status": "Improved",
    "structure_form": null,
    "property_usage_type": "Utility",
...
function mapInstrumentToDeedType(instr) {
  if (!instr || instr.trim().toUpperCase() === "N/A") return null;
  const u = instr.trim().toUpperCase();
  
  // Common abbreviations and variations
  if (u === "WD" || u.includes("WARRANTY")) return "Warranty Deed";
  if (u === "SW" || u.includes("SPECIAL WARRANTY")) return "Special Warranty Deed";
  if (u === "QC" || u.includes("QUITCLAIM") || u.includes("QUIT CLAIM")) return "Quitclaim Deed";
  if (u === "GD" || u.includes("GRANT")) return "Grant Deed";
  if (u.includes("BARGAIN") && u.includes("SALE")) return "Bargain and Sale Deed";
  if (u.includes("LADY BIRD") || u.includes("ENHANCED LIFE")) return "Lady Bird Deed";
  if (u.includes("TRANSFER ON DEATH") || u === "TOD") return "Transfer on Death Deed";
  if (u.includes("SHERIFF")) return "Sheriff's Deed";
  if (u === "TD" || u.includes("TAX DEED")) return "Tax Deed";
  if (u.includes("TRUSTEE")) return "Trustee's Deed";
  if (u.includes("PERSONAL REP") || u.includes("PR DEED")) return "Personal Representative Deed";
  if (u.includes("CORRECTION")) return "Correction Deed";
  if (u.includes("DEED IN LIEU")) return "Deed in Lieu of Foreclosure";
  if (u.includes("LIFE ESTATE")) return "Life Estate Deed";
  if (u.includes("JOINT TENANCY")) return "Joint Tenancy Deed";
  if (u.includes("TENANCY IN COMMON")) return "Tenancy in Common Deed";
  if (u.includes("COMMUNITY PROPERTY")) return "Community Property Deed";
  if (u.includes("GIFT")) return "Gift Deed";
  if (u.includes("INTERSPOUSAL")) return "Interspousal Transfer Deed";
  ...
  if (u.includes("CONTRACT FOR DEED")) return "Contract for Deed";
  if (u.includes("QUIET TITLE")) return "Quiet Title Deed";
  ...
  if (u.includes("RIGHT OF WAY")) return "Right of Way Deed";
  if (u.includes("VACATION OF PLAT")) return "Vacation of Plat Deed";
  if (u.includes("ASSIGNMENT OF CONTRACT")) return "Assignment of Contract";
  if (u.includes("RELEASE OF CONTRACT")) return "Release of Contract";
...
function extractValuation($) {
  ...
  return years.map(({ year, idx }) => {
    ...
    return {
      ...
      agricultural: get("Agricultural (Market) Value"),
    };
  });
}
...
function writeProperty($, parcelId) {
  ...
  if (!propertyFields?.property_type || propertyFields.property_type === null) {
    throw {
      type: "error",
...
function writeSalesDeedsFilesAndRelationships($) {
  ...
  sales.forEach((s, i) => {
    ...
    let page = null;
    if (s.bookPage) {
      const bookPageText = s.bookPage.split("\n")[0].trim();
    ...
    // Only create deed if at least one field is present
    if (deedType || book || page) {
      ...
      // Only create file if link is present
      if (s.link) {
...
function writeTaxes($) {
  ...
  vals.forEach((v) => {
    const taxObj = {
      ...
      agricultural_valuation_amount: parseCurrencyToNumber(v.agricultural),
...
function attemptWriteAddressAndGeometry(unnorm, secTwpRng) {
  ...
  const county_name = inputCounty || "Liberty" || null;
...
function createStructureFiles(seed,parcelIdentifier) {
  ...
  if (structuresData && parcelIdentifier) {
    console.log("INSIDE struc")
    const key = `property_${parcelIdentifier}`;
    console.log("key", key)
    const structures = structuresData[key] || [];
...
function createUtilitiesFiles(seed,parcelIdentifier){
  ...
  if (utilitiesData && parcelIdentifier) {
    const key = `property_${parcelIdentifier}`;
    const utilities = utilitiesData[key] || [];
...
function extractAcreage($) {
  ...
  $(OVERALL_DETAILS_TABLE_SELECTOR).each((_, tr) => {
    const label = textTrim($(tr).find("td strong").first().text()) || textTrim($(tr).find("th strong").first().text());
    if (label === "Acreage") {
      acreage = textTrim($(tr).find("td span").first().text());
    }
  });
  return acreage;
}

function createLotData(parcelId, acreage) {
  const acreageNum = toInt(acreage) || parseCurrencyToNumber(acreage);
  return {
    source_http_request: {
      method: "GET",
      url: seed?.source_http_request?.url || "https://example.com/property"
    },
    request_identifier: parcelId,
    lot_type: acreageNum > 0.25 ? "GreaterThanOneQuarterAcre" : "LessThanOrEqualToOneQuarterAcre",
    lot_size_acre: acreageNum || null,
    lot_area_sqft: acreageNum ? Math.round(acreageNum * 43560) : null,
...
function extractExtraFeatures($) {
  const detected = { structure: {}, lot: {}, utility: {} };

  $(EXTRA_FEATURE_TABLE).each((_, tr) => {
    const tds = $(tr).find("td");
    if (!tds.length) return;
    const description = textTrim($(tds[0]).text()).toUpperCase();
    if (!description) return;

    if (description.includes("SEPTIC") && !detected.utility.sewer_type) {
      detected.utility.sewer_type = "Septic";
    }

    if (description.includes("WELL") && !detected.utility.water_source_type) {
      detected.utility.water_source_type = "Well";
    }

    if (description.includes("EXTRA HP") && !detected.utility.heating_system_type) {
      detected.utility.heating_system_type = "HeatPump";
    }

    if (description.includes("CONCRETE") && !detected.lot.paving_type) {
      detected.lot.paving_type = "Concrete";
    }

    if (description.includes("CLFENCE") && !detected.lot.fencing_type) {
      detected.lot.fencing_type = "ChainLink";
    }

    if (description.includes("SHED") && !detected.structure.roof_design_type) {
      detected.structure.roof_design_type = "Shed";
    }
  });
...
function writePropertyUtilityLotStructureFromExtraFeatures($, parcelId) {
  const acreage = extractAcreage($);
  const detected = extractExtraFeatures($);
  const acreageNum = toInt(acreage) || parseCurrencyToNumber(acreage);
  ...
  const lotData = {
    ...
    paving_type: detected.lot.paving_type || "None",
    site_lighting_fixture_count: null,
    site_lighting_installation_date: null,
    site_lighting_type: detected.lot.site_lighting_type || "None",
    ...detected.lot,
  };
  
  writeJSON(path.join("data", "lot.json"), lotData);
  
  // Create property-lot relationship
  writeJSON(path.join("data", "relationship_property_has_lot.json"), {
    from: { "/": "./property.json" },
    to: { "/": "./lot.json" }
  });
  
  if (hasValues(detected.structure)) {
    ...
    writeJSON(path.join("data", "structure.json"), structureRecord);
    
    // Create property-structure relationship
    writeJSON(path.join("data", "relationship_property_has_structure.json"), {
      from: { "/": "./property.json" },
      to: { "/": "./structure.json" }
    });
  }
...
function main() {
  ...
  if (parcelId) {
    const parcel = {
      ...
      request_identifier: propertySeed?.request_identifier || parcelId || "",
      parcel_identifier: parcelId || ""
    };
